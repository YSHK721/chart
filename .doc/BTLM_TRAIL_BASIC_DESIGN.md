...
> する新規オーバーレイ指標。名目 ols バンドと経験分位バンドの 2 方式・任意分位ペア・外れ値オフセット
> ライン・実現被覆率／β／残差 σ の数値表示を持つ。`tgp_btlm` 本体のソース 8 択化を同梱変更として含む。
> ステータス: 基本設計（実装未着手）。
...
- **達成目標**：
  ...
  3. 任意分位ペアを追加でき、複数ペアを同時にバンド描画できる。
  4. 実現被覆率（分位ペアごと）・β・残差 σ を数値表示する。
  5. MA 参考線（`btlm_mean` のみ・既定オフ）を `moving_averages` と同期して重畳可能にする。
...
| FR-04 | 任意分位ペアを追加可能にする（既定 `[0.05, 0.95]`・`0<q_low<q_high<1` 検証・複数ペア→複数バンド同時描画） | plan §2（分位パラメータ） |
| FR-05 | 分位ペアを帯として表示可能にする（ライン／塗りエリア選択・既定オフ） | plan §3（バンド表示） |
| FR-06 | 外れ値オフセットライン（バンド端から任意 %・上下対称・既定オフ）を描画可能にする | plan §3（外れ値オフセットライン） |
...
| FR-10 | MA 参考線（適用対象 `btlm_mean` のみ・既定オフ・種別／ソースは `moving_averages` と同期）を重畳可能にする | plan §3（MA 参考線） |
...
★[usecase] TrailComputer               ← 各バー t で直近 maxbars 本に OLS をローリング当てはめ、
   ...
   窓末尾値 3 系列（btlm_mean／分位下端／上端・時系列）＋ β 系列 ＋ σ 系列
        ...
        │
        ├─▶ ★[usecase] OffsetLiner     ← バンド端から任意 %（上下対称・既定オフ）の補助線
        │
        ▼（表示 ON 時のみ）
★[usecase] TrailMaSmoother             ← moving_averages.core の MA 関数を参照（btlm_mean のみ）
        │   （sma/ema/smma/lwma・種別/ソース同期・既定オフ）
        ▼
★[adapter] 描画                        ← lwc へ系列追加（ドット/ライン・帯・オフセット線・MA 線）
                                          ＋ β・実現被覆率・残差 σ を数値表示
```
...
| F-06 | 任意分位ペア | 分位ペア追加（既定 [0.05,0.95]・検証・複数ペア→複数バンド） | 高 | FR-04 |
| F-07 | 帯表示 | 分位ペアを帯（ライン／塗りエリア・既定オフ）で表示 | 中 | FR-05 |
| F-08 | 外れ値オフセットライン | バンド端から任意 %（上下対称・既定オフ）の補助線 | 中 | FR-06 |
| F-09 | 実現被覆率算出 | 分位ペアごと直近 N バー（既定 250）ローリング被覆率を数値表示 | 高 | FR-07 |
| F-10 | β／残差 σ 表示 | 傾き β・残差 σ を数値表示 | 中 | FR-08, FR-09 |
| F-11 | MA 参考線 | `btlm_mean` のみ・既定オフ・種別/ソース同期 | 低 | FR-10 |
...
- **任意分位ペア（FR-04）**：分位ペアを任意に追加可能。既定 `[0.05, 0.95]`。各ペアは `0 < q_low < q_high < 1`
  を検証する（bands.py:61-62 の制約と同一定義）。複数ペア指定時は複数バンドを同時描画する。
  ols 方式は `norm_ppf(q)`、経験分位方式は経験 q をそのまま適用する（参照実装 `build_btlm_bands` の
  `q_low/q_high` 引数をそのまま利用＝無改変）。
...
- バンド端から任意 %（ユーザー入力・既定オフ・plan §3）外側に補助線を上下対称に描画する。
- 用途はストップ距離の可視化（`FINDINGS 結論 E`：24h・90% 生存 ≒ 2.77% を設定 UI の説明に参考併記）。
  これは「刈られない距離」であり「儲かる距離」ではない（標本 79 件で推定が粗い旨も併記・§9.4）。

#### F-09 実現被覆率算出（分位ペアごと）

- **定義**：分位ペア `(q_low, q_high)` が各確定バーで作る帯 `[L_t, U_t]` に対し、直近 N 確定バー
  （既定 250・パラメータ・plan §2）のうち「確定バーの `close` が当日帯 `[L_t, U_t]` 内」であった割合を
  ローリング算出する（plan §2「確定日 close が当日バンド内」）。分位ペアごとに独立に算出する。
- **バンドは校正しない**（plan §2）。被覆率は表示値として観測する用途に限る。
...
- 適用対象は `btlm_mean` のみ・既定オフ（plan §3）。種別（`sma`/`ema`/`smma`/`lwma`・catalog.js:228）と
  ソース（8 択）は `moving_averages` と同期する。
- **計算**：`moving_averages.core` のバッファ関数を参照して連結 `btlm_mean` 系列へ適用する。warm-up マスクは
  `moving_averages` の慣行に対称化する。
- **位置付け（実証）**：方向確認用の参考線（β の劣化版）。ドット系列に MA を重ねる二重平滑は日次の値動きとの
  連動をほぼ消す（相関 0.02・`FINDINGS §4「MA について確定した事実」`）。方向一致率は 21 本で 73.8%
  （`FINDINGS §4` 補助行）。
...
| 外れ値オフセット系列 | 帯端から任意 % 上下対称の補助線 | offset_pct、上端／下端 value | バンドに依存（表示 ON 時のみ） |
...
| `offset_pct` | FLOAT | 0（オフ） | ≥ 0（上下対称） | FR-06・plan §3 |
