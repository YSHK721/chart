...
- 作成日：2026-08-09
- 最終更新：2026-08-10
- バージョン：v0.3.2（基本設計・**段階 1／段階 2／段階 3 は実装済み**・段階 4 は未実装）
...
| U-9 | `app.css` の hex リテラルは 166 個・相異 21 種前後 | 6 桁 hex **280 出現 / 相異 36 種**、8 桁 hex **1 出現 / 相異 1 種**、3 桁 hex **0 出現**、数値 `rgba()` **23 出現 / 相異 10 表記**（E-21・v0.3.4 再訂正 2026-09-08。旧記載「合計 219 出現 / 相異 34 種」は 8 桁 hex の取りこぼしと段階 3 追加分の陳腐化を含んでいた＝ISSUE-359） | **是正して採用**（重複が支配的という結論は不変・むしろ強化） |
...
| # | 事実 | 実証根拠 |
|---|---|---|
| E-21 | `app.css` は色リテラルの塊であり、**同一値の重複が支配的**である。**（v0.3.4 是正・2026-09-08 再計測）** 6 桁 hex **280 出現 / 相異 36 種**、8 桁 hex **1 出現 / 相異 1 種**（`:306` `#2962ff22`）、3 桁 hex **0 出現**、数値 `rgba()` **23 出現 / 相異 10 表記**（hex+rgba 合計 **304 出現**）。主要値の出現数は `#2a2e39` 60・`#ffffff` 37・`#d1d4dc` 36・`#2962ff` 23・`#787b86` 23・`#131722` 10・`#ef5350` 7・`#26a69a` 1。v0.2.0 の旧計測（合計 219 出現 / 相異 34 種）には誤りが 2 つあった：(1) 正規表現 `#[0-9a-fA-F]{6}\b` は `\b` が直後の英数字で成立しないため **8 桁 hex を構造的に取りこぼす**＝「全数列挙」は不成立（ISSUE-359）。(2) 段階 3 のテーマ UI 用 CSS 追加以降の増分を含まない陳腐化。なお §2.3・§8 の派生集計（「CSS 219 リテラル」「front JS 50 リテラル」等）は v0.2.0 時点の値のままである | 8 桁を先読みで排他した 4 形式の全数計測（2026-09-08・`indigators/indicator_ui/web/css/app.css` 1 実体）：`#[0-9a-fA-F]{8}(?![0-9a-fA-F])`（1 件）・`#[0-9a-fA-F]{6}(?![0-9a-fA-F])`（280 件）・`#[0-9a-fA-F]{3}(?![0-9a-fA-F])`（0 件）・数値 `rgba()`＝`rgba?\(...\)` のうち `var()` を含まない式（23 件）。**出現数はコメント中のリテラルとトークン定義行（`--x: #hex` の 5 件）を含む** |
...
#### 4.1.5 指標側 `SeriesDef` 101 件の付与内訳（全数）
...
| 27 | period_hl | 2 | 0 | 0 | 0 | 0 | 0 | 0 | 2 | 0 | 0 | `catalog.js`（期間高安・走行極値＝通常域の縁・ISSUE-490） |
| 28 | ytd_hl | 2 | 0 | 0 | 0 | 2 | 0 | 0 | 0 | 0 | 0 | `catalog.js`（年初来高安・稀な水準＝alert・ISSUE-490） |
| — | **合計** | **101** | **1** | **1** | **4** | **27** | **21** | **6** | **20** | **18** | **3** | 列和 1+1+4+27+21+6+20+18+3 ＝ 101（行和と一致） |

- 動的 `SeriesDef` は 10 件（tgp_btlm・btlm_trail・btlm_trail_marod・ma_marod・profit_rsi・tickvol 各 1 件、profit_band 4 件）。静的は 91 件。
...
- 処理（決定論的・この順序で固定）：
  ...
  2. **クロムを適用する**（先に地を決めてから図を載せる）。
     ...
     - 派生 3 件（§4.2 #18〜#20）を含む**全 20 配線点**を `resolveAllChrome` で解決し、`ChartRenderer` が**保持値として受け取る**。減光ローソク（#18）・分析 tint（#19）・背景フォールバック（#2）・ローソク復元色（#12/#13）は、この保持値からのみ読む（v0.3.1 で明確化）。
       ...
       - **クロム出力は「保持色 × 表示モード」からの導出 1 本で決める**（v0.3.1 で追記・ISSUE-357）。表示モード（ローソク透明化・ペア外減光・分析 tint）は `_chromeSlots` と並ぶ入力であり、どちらが変わっても同じ規則で出力を再計算する。書き手をモードごとに分けると、テーマ適用が透明化や tint を無言で破壊する（実測済み）。
...
- **再計算を行わない**。`/compute` を呼ばない。指標系列の色は既存系列への `applyOptions` で完結する（`series_drawer.js:385`）。
  - **`setData` の扱い（v0.3.1 で是正）**：per-point の色で表現している要素（ペア外の減光ローソク）は `applyOptions` では色を変えられないため、その要素に限り `setData` で**色だけ**を書き直す。許されるのは §3.4 の解釈どおり「自分が所有する per-bar 色の塗り直し」であり、**書き戻す対象は現在所有しているバー集合に限る**（トリム中なら T までのバーだけ）。全件を書き戻すと「どのバーが存在するか」まで変わり、T より後のバーが再表示される（実測で発生した欠陥・ISSUE-357 の周辺）。減光が無効なときは `setData` を一切呼ばない。
...
- 開閉挙動は時間足・テンプレートメニューと同一（**ツールバーのドロップダウンは同時に 1 つしか開かない**＝他のトリガーを押したときも「外側クリック」として閉じる。v0.3.2・ISSUE-366。基本設計_チャートテンプレート §6.2 と同文）。
