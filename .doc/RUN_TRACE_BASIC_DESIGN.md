...
## 9. 段階 4: 分析面（設計・工程 2 の事実確定を経て 2026-09-10 確定）

**設計を書く前に工程 2（読み取り専用）で事実を確定させた**。理由: 本設計書の前提はこのセッションで
既に 4 回実測に崩されており（§5.1・§6.4 の 2 件・§7.0）、書いてから崩されるより先に測るほうが安い。
その結果、着手前に**前提の崩れが 5 件**見つかった。以下はその実測を織り込んだ設計である。

### 9.0 先に直す欠陥: `time` 列が秒精度で、実ティックの 25.8% が潰れる

**実測（2026-09-10）**:

| 事実 | 出典 |
|---|---|
| tick store の `timestamp` は `datetime64[ms, UTC]`＝**ミリ秒精度** | `data/marketdata/ticks/2025/01/*/JP225_ticks.parquet` |
| **実 run 実測（工程 3・2026-09-11）**: JP225 2026-01・実ティック **1,036,394 点**のうち、秒へ丸めると **407,745 行（39.3%）が同値**になる。**同一秒を最大 14 行が共有**する | trace ON の実 run |
| （初期の見積り 25.8% は 1 日分 733 行の標本。実規模では **1.5 倍**だった） | 標本サイズの差 |
| `time` 列は `epoch_seconds()` を通る＝`_from_numpy_datetime64` が `.astype("datetime64[s]")` で**切り捨てる** | `simulator/adapter/trace/columnar_run_trace.py`・`simulator/domain/bar_time.py` |

依頼者の要求は「**ティック粒度での推移**で全ての項目を保存」である。行そのものは
`(bar_index, tick_ordinal)` で区別できるが、**時間軸としては 4 行に 1 行が同じ時刻になる**。
これは要件に対する欠陥であり、分析面の側で回避する（同一秒を代表値へ潰す等）のは
**症状の出る条件を避ける形＝対症療法**である。既存 front の `chart.js:47-51 dedupeCurve` が
まさにその形で、流用すると**同一秒内の equity / margin_level の谷が消える**（DD 分析が壊れる）。

**裁定: `time` 列を epoch ミリ秒（int64）にする。**

- 変換規則は `simulator/domain/bar_time.py` に **`epoch_millis` を加法**する。
  解釈規則（`EPOCH_CONVERTERS` の受理集合）は**1 つのまま**で、出力単位が 2 つになるだけである
  （「どの表現を受けるか」と「どの単位で出すか」は別の関心）
- tick store が `datetime64[ms]` なので**無損失**。ブラウザの `Date` もミリ秒なので front で変換が要らない
- `trades` の `entry_time` / `exit_time` は epoch 秒なので、突合は ×1000 で行う
- **既存データは壊れない**: `trace_*.parquet` は実在が 0 件（工程 2 実測）。消費側も未実装

### 9.1 置き場所（工程 2 が 8 候補を実測し、成立するのは 3 つ）

| 候補 | 判定（実測） |
|---|---|
| dashboard（第 4 モード）へ置く | **成立しない**。dashboard core はジョブ台帳も `/data/{job}` 配信口も持たず、`report.json` を 1 件も参照していない。置けば台帳の 2 つ目の所有者ができる |
| 既存 3 窓の下へ 4 窓目 | **成立しない**。既存 3 窓は**バー時刻**で `syncLogicalRange` しており、ティック時刻系列は同じ時間ドメインに乗らない |
| 第 5 モード（新 core） | 成立するが重い（新プロセス・新ポート・`serve.sh` 改訂・router の upstream 表） |
| **sim 面の 5 枚目のタブ ＋ サーバ側 JSON API** | **採用**（新設が最小・既存の器と配信骨格を借りられる） |

**既存面との関係（重要な事実）**: 既存の「Balance」「Drawdown」窓は**在るが別物**である。
中身は `agg.balance_curve`（確定トレードの `exit_time` × 走行残高）をバー時刻へ前方補完したもので、
窓のラベル自身が「**残高ベース**DD」と名乗っている。**equity（含み損益込み）・証拠金・維持率・
保有玉数はどの成果物にも出ていない**（`report.json` にも `stats.json` にも無い）。
したがって段階 4 は「既存面の拡張」でも「新設」でもなく、**既存 3 窓の隣に別の量を並べる**。

### 9.2 front へ渡す量: サーバが窓を受けて必要ぶんだけ返す

**front に parquet を読む手段はリポジトリ内に 1 つも無い**（`package.json` の依存 0・vendor は
lightweight-charts と chart.umd のみ）。front で直接読む案は**ライブラリ追加＝承認事項**を必ず伴う。

**裁定: サーバ側の JSON GET ルート（`api_shared/json_get_routes.GetRouteResponder`）で、
窓と列を受けて必要ぶんだけ返す。**

**「粒度」を受ける案は撤回する**（工程 3 の指摘・初版の誤り）。サーバ側の間引き・集約は
**必ず「読んで捨てる」**ことになり、§9.6 の不変条件「読んだ行数 − 返した行数 = 0」と**両立しない**。
上限を超える要求は**黙って間引かず 413 で明示的に断る**。俯瞰用のバケット集約（min/max 包絡）が
要るなら、それは「捨てない集約」の設計を伴う**別裁定**である。

**初版の暗黙の前提 2 つが実測で崩れた**:

| 初版が前提にしていたこと | 実測 |
|---|---|
| front が表示幅から窓を計算できる | **偽**。実 1M 行で比例配分すると 59,030 行（上限 20,000 の 2.95 倍）になり初回表示が 413 になる。→ **サーバが `suggested_window` を測って答える**（10 probe・253 ms・行の materialise 0） |
| GET の JSON ルートはクエリ文字列を受けられる | **偽**。`serve_sim.py` が `urlparse(self.path).path` を使うためクエリが落ちる（既存 3 本も同様）。→ **窓はパスセグメントで運ぶ** |

**非有限値の `null` 化は応答境界 1 箇所で行う（フィールドごとに撒かない・工程 5 レビュー 🔴-4）**。
工程 3 は `_points` の 4 群へ手書きで適用し、**`/extent` に適用し忘れた**。実測で
`initial_deposit=inf` / `margin_level_floor=nan` が `Infinity` / `NaN` として直列化され、
ブラウザの `JSON.parse` が `SyntaxError` になる。**front は `/extent` を最初に叩く**ので、
落ちると分析タブは全面が掲示のみになる。しかも当該検定は有限値 fixture のため**原理的に落ちない**
（見かけの被覆）。

これは拡張点の欠如＝OCP 違反である。**応答の唯一の出口で payload 木を再帰的に通す**形にし、
フィールドを増やしても取り落ちないようにする。検定は `/extent` へ非有限値を注入する 1 本を足す。

**`margin_level` の `inf` は `null` へ落として返す**（工程 3 の pre-mortem で検出）。
実 run の **39.2%（405,941 行）** が `inf`（`margin == 0` のとき無限大＝`domain/account.py` の契約）。
Python の `json.loads` は `Infinity` を受理するが **ブラウザの `JSON.parse` は `SyntaxError`** になる。
Python 側の検定だけでは全部緑のまま本番で落ちる形なので、**JSON の厳格パースで検定する**。

理由は絶対命令そのものである。952,832 行を渡して画面が数千点しか描かない形は
**「作ってから捨てる」**であり、規約上通らない。先例も在る——
`dashboard_ui/web/js/domain/ladder_window.js` は「窓の外は**建てない**——建ててから隠すと
捨てる色計算が毎描画発生する」と明記し、`sliceWindow` で建てる範囲を先に決めている。

配信骨格は既存を借りる（`GetRouteResponder` の先例 3 本・`composition_root_display.py` の
委譲による OCP 包み）。**run 時に固定した派生成果物を書く案は採らない**——「何を見たいか」が
run 時に決まらず、窓・列・粒度を固定してしまう。

### 9.3 アクター単位の分割（SRP）

| モジュール | 層 | アクター（改訂の動機） |
|---|---|---|
| `adapter/trace/parquet_trace_store.py`（**加法**） | adapter | **どう読むか**（列 pushdown・窓スライス）。pandas/pyarrow の唯一の到達点という既存宣言と一致 |
| `sim_ui/usecase/trace_query_ports.py`（新規） | usecase | **読み取りの契約**（DIP。usecase は adapter を掴めない＝層ゲート） |
| `sim_ui/usecase/query_trace.py`（新規） | usecase | **何を返すか**（窓・列・粒度の解釈と、返す量の決定） |
| `sim_ui/usecase/derive_trace_events.py`（新規） | usecase | **事象の導出**（halt 遷移・保有数の変化・維持率の閾値割れ）。trace は**状態列**であって事象列ではない |
| `sim_ui/adapter/trace_query_source.py`（新規） | adapter | Port の実装（store への束縛） |
| `sim_ui/adapter/trace_api_controller.py`（新規） | adapter | HTTP 面（JSON の形） |
| front: 分析タブ 1 枚 | front adapter | **見せ方** |

### 9.4 借りるもの（新規実装を作らない）

- **equity DD の算出式**: `usecase/mt5_parity.py` の `equity_dd_absolute` / `_maximal` / `_maximal_percent`。
  `equity` 列を float 列で渡すだけでよい（`sim_ui/usecase` と `simulator/usecase` は**同じ usecase 層**＝相互 import は層ゲート上合法）
- **balance DD の算出式**: `usecase/metrics_spec.py`
- ジョブ結果の配信口・完了判定・部分結果の非公開・行→チャート連動（`linkage.js`）・
  meta による誤読防止（`indicator_bar_index_is_comparable`）・部分成果物の判別規則（§6.5.2.4）

### 9.5 侵してはならない制約（工程 2 実測）

| ゲート | 要求 |
|---|---|
| `test_run_job_trace.py` の byte 等価 | `stats.json` / `report.md` は trace ON/OFF で 1 バイトも変わらない。**段階 4 の出力を `report.json` へ相乗りさせない** |
| 指紋 A/B/C | run の結果が 1 bit も動かない |
| `sim_ui/web/tests/removed_ui_vocabulary_gate.test.js` | front の `js/adapter/front/*.js` に `trailing` / `partial` の語を**コメント含め**書けない。事象名にこれらの語を使わない |
| `test_tick_parquet.py` | `simulator/usecase/*.py` に pandas/pyarrow 0 件（`sim_ui/usecase` は対象外） |
| 層順序・循環 0・`sim_ui/**`→`simulator.main` 禁止 | 従来どおり |

**タブ宣言の手書き写しは根本是正する（所有者は 2 人いる・工程 5 レビュー 🔴-1）**。
初版は `sim_tabs_view.test.js` だけを名指したが、**同じ手書き写しが e2e にもある**:
`sim_ui/tests/e2e/verify_sim_display_parity_peripheral.py` の
`common = {"detail","heat","compare","glossary"}`。しかもこの e2e は**既定の pytest 収集に載らない**
（`verify_*.py` 命名・ISSUE-378 #2 が OPEN で既知）ため、**赤が自動では見えない**。
1 人目だけ直して 2 人目を残すのは、根本是正を掲げた段階が自分で「手書きが古い」型の赤を作る形である。

**規則**: 宣言（`SIM_TAB_NAMES`）の写しを持つ検定を**全数列挙してから**是正する。
期待値は宣言から導出し、リテラルの集合をどの検定にも書かない。


`SIM_TAB_NAMES` は宣言駆動なのに検定側が `["detail","heat","compare","glossary"]` と
`panes.length == 4` を**手書きで固定**している。タブを 1 枚足すと赤になるが、**期待値の手書きこそが欠陥**である
（`file_job_ledger` の手書き列挙と同型で、同じ根本是正＝宣言からの導出を行う）。
検定は「宣言と生成物が過不足なく一致すること」を表明する形へ変え、名前の一覧をテストへ書かない。

### 9.6 計算量テスト（絶対命令）

- **読んだ行数 − 返した行数 = 0**（窓の外を読まない。列 pushdown が効いている）
- **窓を固定したまま run 長を変えても、読む量・返す量が増えない**（2 点以上で固定）
- **事象の導出が状態列を 1 回だけ走査する**（事象ごとに全走査し直さない）
- いずれも**正の対照**（返した行数 > 0・事象が 0 件でない）を同梱する

**測る計器は 2 つ要る。「発行総数」だけでは不十分**（工程 5 再レビューで私の処方が実測で否定された）。

写しは同じ要素を保持するため、**列を 1 度写してから写しを再走査する浪費は発行総数が変わらない**
（実測: 発行計測単独で当該変異が 3 passed）。必要なのは次の 2 つである。

| 計器 | 捕らえるもの |
|---|---|
| 1. 要素参照の発行総数（`発行 − 使用 == 0`） | **供給元**の再走査 |
| 2. **同一列からの連続発行の最大が 1** | **写し**の再走査（materialise するとブロック排出になり最大 = 行数） |

**計器 2 を「構文木の loop 数」で実装してはならない**（工程 5 再レビューが実測で両方向の誤りを示した）:

| 実装 | 偽陰性 | 偽陽性 |
|---|---|---|
| `loops == 1` | 浪費を助力メソッドへ移すと素通り（最も自然な分解＝**最も起きやすい形**） | 列名を宣言から導いて遅延 `zip` する**正当な改善**を赤にする |
| 同一列の連続発行の最大 == 1 | — | — |

実測値（識別力の確認）:
```
honest                      発行=800  連続発行の最大=  1  events=250
正当な書き換え               発行=800  連続発行の最大=  1  events=250
写し＋助力メソッド（浪費）    発行=800  連続発行の最大=200  events=250
```

`loops == 1` は実装詳細を仕様へ昇格させる形であり、CLAUDE.md が名指しで禁じた
「回数そのものを期待値に焼き込む」（ISSUE-450 の前例）と同型である。

**自己検定の見本は 2 形（インライン形・助力メソッド形）置く**。見本を助力メソッド形にすれば、
検出境界が足りないときに自己検定が自分から赤になる（実測で確認済み）。

**front の到達可能性の検査には陰性対照を付ける**（工程 5 再レビュー 🟡-A）。
観測式を定数化すると production が壊れていても緑になる（実測）。
「面が生えているのは当該ペインだけ」を対照として置く（他ペインは `hasRoot: False` を実測済み）。

**応答の出口段にも計算量ゲートを置く**（同 🟡-B）。`_json_safe` の導入で仕事がこの層へ移ったのに
計器が追随せず、出力を 1 bit も変えない冗長な中間リストが入った（実測 **+93% の一過性割当**）。
「応答の組み立てで materialise した要素数 − 応答が運ぶ要素数 = 0」を表明する。
工程 3 の Spy は注入列への `__iter__` / `__getitem__` を数えたため、**実装が先に写しを作れば
以後の浪費が観測外**になった。実測: 列を 1 度写してから種別数（5）ぶん写しを全走査する変異
（出力は 1 bit も変わらない）が**全緑**で通った。発行 5N・使用 N で等式が破れているのに検定が通る＝
ISSUE-450 と同型である。

**検出力の自己検定を必ず同梱する**（浪費実装を検定内に置き、検出器がそれを赤にすることを表明する）。
先例は同セッションの `sim_ui/web/tests/sim_trace_block.test.js` と
`replay_ui/tests/unit/test_static_file_server_allowlist_complexity.py`。

**front の「呼ばれていること」は実ブラウザで固定する**（工程 5 レビュー 🔴-2）。
静的検査では到達可能性を証明できない（`if (false)` で囲む変異が全ゲートを通る・実測）。
工程 5 が実 chromium で `paneChildren 1 → 0` の識別を実証したので、**証明手段が無いのではなく検査が無い**。
e2e へ 1 アサーション（分析ペインに面が実際に生えること）を足して閉じる。

### 9.7 実測結果（工程 3・2026-09-11 実施済み）

trace ON の実ティック run を 1 本回して測った（JP225 2026-01・**1,036,394 点**）:

| 項目 | 実測 | 初版の見積り |
|---|---|---|
| `trace_points.parquet`（秒精度時） | **7.29 MiB** | §2 実測 7 の 61.6MB ＝ **8.4 倍の過大** |
| 同（ミリ秒化後） | **9.26 MiB**（+27%・時刻が一意になった分の圧縮損） | — |
| run 中 peak RSS | **1,087 MB**（trace OFF は 360 MB・差 **+727 MB**） | §6.5.2.2 の tracemalloc 外挿 139〜387 MB ＝ **過小** |
| `report.json` / `stats.json` / `report.md` | trace ON/OFF で **sha256 同一** | — |

**61.6MB を返す量の設計根拠にしてはならない**（実列は圧縮が効く）。一方 **RSS は外挿より大きい**
（tracemalloc は Python オブジェクトの割当だけを見るため、pandas/pyarrow の C 側バッファを数えない）。
§6.5.2.2 の裁定（窓なし ON は許す／根本策は逐次書出し）は**変えない**が、根拠の数値は本節が上書きする。
- **lightweight-charts v5 が同一 `time` の重複点をどう扱うか**（9.0 の是正後は重複しないが、
  ミリ秒時刻を受けるかは実 UI で確認が要る）
