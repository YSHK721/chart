...
## 追加要件裁定（2026-08-20・依頼者指示＝チャートクリックによる価格入力）

依頼者指示（原文趣旨）: 「価格の入力をチャート上からクリックして入力できる UI にしろ。チャート上の
移動平均線をクリックしたら入力される。移動平均線に限らず、**指標系はクリックしたら入力できる**仕様に
しろ。**チャート上の任意の場所で入力できる**仕様にしろ。」

- **R-P1 基本操作＝アーム式ピッカー**（依頼者採択）: モーダル各価格欄（建値 i／損切り／利確）の
  「チャートで指定」を押すとチャートがピッカーモードになり、クロスヘア追従のゴースト線＋採用予定
  価格を表示。クリックで確定・Esc（またはモーダル側の取消）で解除。入力先が常に一意＝誤入力が
  構造的に起きない。
- **R-P2 スナップ対象**（依頼者採択・拡張指示）: クリック点の近傍（px 許容内）にある
  **表示中の全指標系列の値（当該足）**と**ローソク OHLC（当該足）**を候補とし、最も近い値へ
  スナップ。近傍に候補が無ければ**素のクリック価格**を採用（任意の場所で入力可）。採用予定値は
  ピッカー中のツールチップで明示する。MA 限定ではない。
- **R-P3 追加操作＝右クリックメニュー**（依頼者採択）: チャート上の contextmenu で
  「この価格を損切りに設定／建値に追加／利確に設定」。価格解決は R-P2 と同一のスナップ規則を使う
  （解決器の単一ソース）。
  ※選択回答には「なし（推奨）」と「右クリックメニュー」が同時に含まれていたため、明示追加された
  右クリックメニューの採用を依頼者意図と解釈して記録する（誤りであれば本行の訂正のみで撤回可能）。
- **R-P4 修飾キー割当は不採用**（認知負荷最小化の厳命）。ドラッグ微調整（既設計スライス 4）は併存
  （クリック＝初期配置・ドラッグ＝微調整）。
- **設計への波及**: スナップ解決器（候補列挙＋最近傍選択）は**純関数として domain/usecase 層**に置き
  単体検定する（§10 で削除した SnapPolicy とは別物＝実在の変更要因が発生したため新設が正当）。
  座標→足・系列値の列挙は renderer の公開メソッド追加（lwc API 名は隔離点内に留める・
  `priceAtCoordinate` と同じ規律）。**未検証事項（工程 2 で実測確定）**: (a) クリック x→足 index→
  各指標系列値の取得経路 (b) 当該足 OHLC の取得経路 (c) contextmenu と既存チャート操作の競合有無。

## 工程 2 検証結果（2026-08-20・現ツリー 698b533 での鮮度検証。以下は本文を上書きする）

**判定＝条件付き合格（スライス 0〜5 着手可・6/7 は下記是正の反映が前提）**。引用 60 箇所超を実測し
崩壊 0・行ズレ 4（±1 行）・記載誤り 2（HTML 総行数は 1178 行・router.py の範囲は :220-241）。
参照実装 HTML の式・分岐 16 項目と入力 13 箇所は**全一致**＝スライス 0 の確定入力は現物どおり有効。
JS 実装 0 件・新規ファイル名衝突 0 件・依存方向違反 0 件も再実測済み。

**是正 1（スライス 6）**: 既存ガードの INDEX_HTML 4 枚は
`indicator_ui`・`replay_ui`・**`report_ui`**・`unified_ui`（本文 §調査 3-A の「sim_ui」は誤認。
`simulator/sim_ui/web/index.html` は別に実在する 5 枚目＝chart_app_wiring 非経由・器も不要）。
新規ガード `position_sizing_toolbar_mount.test.js` は INDEX_HTML 一覧を**手書き複製せず**、
既存一覧を単一ソース化して共有する。

**是正 2（スライス 7）**: `served_import_resolution.test.js` と `symlink_health.test.js` の走査根は
`indicator_ui/web/js` **のみ**＝replay 配信根を 1 byte も見ない（symlink 張り忘れが全緑のまま
実 UI 404 になる＝ISSUE-268 同型）。是正＝既存 2 本の**走査根を配列化**して
`simulator/replay_ui/web/js` を加える（新テスト追加ではなく既存ガードの射程拡張）。
スライス 7 の通過条件に「replay 配信根で symlink を 1 本落とすと Red」の実演を追加。

**TBD-1 採用のスライス反映（本文の「要裁定」記述を確定へ読み替える）**:
- スライス 0 の対応元から `:908-913`（offsetsFrom＝gap 専用）と `:964` の gap 分岐を除外。
  権威 API から `mode`（up/down）引数を落とす（direct では両系統同一＝`:1098`）。
- スライス 0 通過条件 1 の正解定義＝「HTML を `pmode='direct'` に切替え、gap が算出した建値列を
  直接入力した状態の出力」（HTML 既定は `:578` `pmode:'gap'` のため、そのままでは突合できない）。
- スライス 6 のモーダルから消える項目＝`:476-479`（pmode）・`:486-487`（g）・`:489-497`（gapmode）・
  順張り/逆張り 2 カード表示。UC-02／E-02／スライス 4 は影響なし（E-02 は元から価格のみ保持）。

起点表記の訂正: 実装起点は develop **1f6e8fe**（本ブランチ 698b533〜）。

## 工程 3 実装確定記録（スライス 0〜2・2026-08-20。本文を上書きする）

実装コミット: `28a867f`（スライス 0＝`simulator/usecase/split_entry_plan.py`・80 検定）／
`3ecc982`（スライス 1＝生成器 2 本・fixture edge 39／split 153 ケース・鮮度検定 4 本）／
`a5e88fb`（スライス 2＝JS domain 4 本＋パリティ検定 4 本・21 検定）。
ゲート実測: pytest simulator **4678**（+84）・web 全 5 スイート緑・変異 3 件 Red→復元緑・
既存資産（参照 HTML・権威 4 本・既存 fixture）差分 0。**TBD-7 は不発**（RoR 2457 点厳密一致・許容 0）。

1. **growth の許容差は「相対 1e-15」を実測で棄却**（実測 1.17e-14。式は 60/60 bit 一致・
   `Math.log` の 1 ULP 差を桁落ちが増幅）。採用＝伝播誤差許容
   `|Δg| ≤ 4·eps·(p|ln(1+Rf)|+q|ln(1−f)|)`（実測最悪値は許容の 23%・変異検定で 12 桁の検出余裕）。
   本文スライス 2 通過条件 2 の「相対 1e-15」はこれに読み替える。
2. **総和は Neumaier 補正**: CPython 3.12+ の組込み `sum()` は補正総和（4000 件差分検定＝素朴加算
   1049/4000 不一致・Neumaier 0/4000）。`account_margin_core.js` は Neumaier で写す。
   golden 再生成時の Python 処理系変更は再検証が要る（fixture 依存の固有性質として記録）。
3. **複製禁止 ＞ import 0**: `split_entry_plan.js` は式を書き下さず `./account_margin_core.js` を
   import（domain 内 1 本＝層規約「domain: 自パッケージ内のみ」に適合・既存 domain にも同種 3 本）。
   本文 §8「domain/* import 0」はこれに読み替える。
4. **UC-03 入力の補完**: `fraction`（採用 f）を明示入力に追加（`L₁=f·E/(V·Σwᵢdᵢ)` に必須）。
   `take_price` 有効判定の P₀ は **`entry_prices[0]`**（参照実装 `:359` ラベル「第1建値 P₀」＋
   `:931` direct 既定シードによる同定）。チャートから任意順で建値を拾い先頭が最値でない場合の
   有効判定はこの定義に従う（参照実装の条件を足さない・削らない）。
...
7. **裁定済（2026-08-20 依頼者承認）**: オシレーターペイン上のクリックは**無効化＋案内**
   （価格ペインのみ入力有効。下段ペインのクリックは確定せず「価格チャート上で指定してください」を
   ツールチップ表示）。

## 工程 3 実装確定記録（スライス 8-a・8-b・6・2026-08-20。本文を上書きする）

実装コミット: `1b78e81`（8-a `snap_price_resolver.js`＝最近傍・許容閉区間・同距離は候補配列先頭・
非有限フェイルクローズ）／`7396056`（8-b renderer 追加 2 面＝barInfoAt :967 直後へ 74 行挿入のみ・
候補列挙順は系列→水準線→OHLC で固定・ペイン幾何は凡例 DTO と同一根拠）／`feceee6`・`07399cb`
（6＝空マウント 1 行＋menu/dialog・import 0・コールバック注入・gap 系/2 カード/図 3 の不在を検定固定・
「チャートで指定」受け口＋`setPrice(target, price)` 書き戻し口）／`8a727b4`・`8e4485d`（追補）。
ゲート: web 全緑（indicator_ui 2066）・pytest 4678・変異 13 件 Red→復元緑・
index.html 4 枚／barInfoAt 検定／simulator／参照 HTML 差分 0。
工程 2 是正 1 は `tests/index_html_pages.js`（4 枚一覧の単一ソース）で実装（既存ガードの
アサーション差分 0・共有を変異で実証）。

- 乖離記録 1（**8-c/8-d/7 で解消すること**）: `exitMode`（決済 ブラケット/時間）の下流受け口が無い
  （usecase/domain に exit 参照 0 件）。**参照実装 HTML の :387-390 の exit が build()/表示にどう
  効くかを読み、その定義に厳密に従って結線する**（推測で「time＝利確なし」等と決めない）。
- 乖離記録 2（採用）: 方向 long/short は `PriceLevels` が保持し VM は `levelLines.direction` で返す
  （設計 UC-03 の入力列挙を実装の事実へ読み替え）。
- 乖離記録 3: オシレーターペインの案内表示 UI は 8-d の責務（判定面は 8-b で用意済み）。
...
| ID | 事項 | 既定案 | 根拠 |
|---|---|---|---|
| A-1 | JP225 の `tick` の値 | **1.0** | 対象は OANDA 証券 JP225 **CFD**（`contract_size=1`・`prototype_260811-01/parity_check.py:11`）。`sim_ui` の `digits=1`/`point_size=0.1` は OANDA-Japan **MT5** JP225 であり本件の権威にならない。<br>**⚠ 注記（2026-08-26・ISSUE-445 段階 3-E2b）: 本行の「別商品」の論拠は崩れている。** 起票時（2026-08-20）はここに「MT5 側は `contract_size=10`（出典 `case.yaml:13-20`）だから CFD（`contract_size=1`）とは別商品」と書いていた。しかし ISSUE-445 の実測で **MT5 JP225 の真値も `contract_size=1.0`** であることが確定した（権威: `marketdata/symbol_specs/OANDA-Japan-MT5-Live/JP225.json`・`trade_contract_size=1.0`）。`10` は出所の無い逆算値（RC-1）だった。よって「contract_size が違うから別商品」という根拠は**消滅した**。これは両者が同一商品であることの証明ではない（同一性は依然として未証明・TBD-D / ISSUE-445 段階 3-F）。**A-1 の既定値 1.0 自体は下記の安全側の論拠で独立に支持されるため変更しないが、「別商品」を前提にした議論を本行から引用してはならない。** なお `case.yaml` の銘柄仕様は段階 3-E2a で撤去済みで、旧引用 `case.yaml:13-20` はもう存在しない。<br>CFD の呼び値の出典はリポジトリ内に無い（`docs/oanda_indices_cfd_about.md:104` は「銘柄毎に異なる（取扱銘柄ページ参照）」・出典は外部 URL `:286`）。未確定下では 1.0 が安全側（1.0 の倍数は 0.1 の倍数でもある／逆は無効価格を作る）。参照実装の価格表示も整数・入力 `step="10"` |
...
## 工程 5 レビュー結果と追加裁定（2026-08-20）

### 判定: 承認（🔴 必須修正 0 件・🟡 推奨 7 件）

レビューが最も価値ある指摘をした点を先に記す。**フェイルセーフ分岐は本番配線から到達できない。** 配信 3 ページはすべて `datasetRef='jp225_tick'`（`indicator_ui/web/index.html:54`・`replay_ui/web/index.html:57,66`・`unified_ui/web/js/unified_root.js:35`）で台帳に必ずヒットするため、`no_symbol_spec` 経路と `digits ≠ 0` 経路は 2309 件の緑が一度も通っていない。**「テストが緑」と「本番で働く」の距離**が、ISSUE-291（受け口だけ作って front が送らず無言で死んだ）と逆向きに現れた形。

**プロセスへの反映**: スライスの通過条件に「その分岐が本番配線から到達可能であることの実証」を加える。

### 実 UI 実測（2026-08-20・ライブ 8000/live・リプレイ 8000/replay の両方）

| 確認項目 | 結果 |
|---|---|
| **元のバグ（生値の書き戻し）** | **解消**。ゴースト `59,558` → 損切り欄 `59558`（生値 `59557.71…` ではない） |
| ツールバー銘柄名 | `JP225`（`NI225` は DOM に 0 件） |
| 価格軸 | `73000 / 72000 / … / 59000` の整数。価格タグも `66868 / 65706` 等すべて整数 |
| 価格入力欄 | `step="1"`（`'any'` から台帳の `tick` へ） |
| 右クリック | 4 項目。実行で損切り欄に `61853`（整数）。項目クリックでモーダルが自動で開く |
| 下段ペインの右クリック | トースト「価格チャート上で指定してください」 |
| 手入力の同期 | `58700.4` → blur → `58700`／空欄は空欄のまま（`null` 維持） |
| コンソールエラー | `favicon.ico` と `data/trade_markers.json` の 404 の 2 件のみ（本変更と無関係） |
| **A-3 の未達（発見）** | `#current-price` が `65,721.051`・バー情報が `O 65,800.949` と**小数 3 桁のまま**。原因は `adapter/front/format.js` の `fmtValue` が `maximumFractionDigits: 3` 固定で 5 モジュールに共有されていること |

### 追加裁定（依頼者・2026-08-20）

| ID | 事項 | 裁定 | 根拠 |
|---|---|---|---|
| **B-1** | 現在値の表示桁（A-3 の未達） | **現在値＋バー情報 OHLC を `digits` に合わせる。指標レジェンドの値は対象外** | 現在値とバー情報は同一オーバーレイに並ぶ同一銘柄の価格で、片方だけ整数にすると見た目が割れる。指標値（`RSI 56.462`・`ma_marod 0.101`）は指標の出力であり、下段ペインには価格でない系列があるため、価格の桁を強制すると誤りになる |
| **B-2** | モーダル初期値の参照実装との乖離（🟡-7） | **価格欄が空で開く現状を正とする**。設計書の乖離記録として明記する | 参照実装は `P0 value="58700"` を持つが、front へ直書きすると「front が価格を自称する」構造（本ブランチが除去した原因 α）を再導入する。現行チャート（JP225 ≈ 65,700）に対し `58700` は意味を持たない。**価格はチャートから拾う**のが ISSUE-368 の趣旨 |

### 乖離記録 4（参照実装との差・意図的）

参照実装 `integrated_position_sizing_calculator.html:359` は `<input id="P0" value="58700" step="10">` を持ち、開いた瞬間に Step 3 が完全に表示される。本実装は価格欄が空（`position_sizing_dialog.js:132-141` の `defaultLevels()` が `null` を返す）で開き、価格が揃うまで Step 3 は「—」のまま。**裁定 B-2 により意図的な乖離**とする。

### マージ前に是正する項目

| 項目 | 内容 |
|---|---|
| B-1 | 現在値・バー情報 OHLC の表示桁（**承認済み A-3 の未達**） |
| 🟡-1 | アーム中に `NO_PRICE` の座標をクリックすると案内が**空文字**で無音（`price_pick_controller.js:168`）。右クリック経路は `MSG_NO_PRICE` を出しており非対称。実装が同ファイル `:178` のコメント「黙って何も起きない状態にしない」に反している |
| 🟡-2 | 水準線 drag だけフェイルセーフの告知が無い（`chart_app_wiring.js:582`）。掴めない理由が出ず、バグと区別できない |
| 🟡-3 | 「刻み上にない価格は存在できない」と謳う関門（`price_levels.js`）が `usableTick` を呼ばない。`tick=0/-1/NaN` で無音の誤答（`NaN`・丸まり）になる。本番配線からは到達しないが、宣言と実装の強度が一致していない |
| 🟡-6a | `symbol_spec.py:24`「変更点は台帳 1 行のみ」は不成立（実測 ≥5 ファイル・≥11 箇所）。記述を事実へ訂正 |
| §6-1 段階 1 | `chart_app_wiring.js:481-483`「テスト改変なしには成立しない」は誤り。`symbolSpec = lookupSymbolSpec(datasetRef)` の既定値つき引数で既存テスト改変 0 のまま成立する。記述を事実へ訂正 |
| 🟡-5 | JS `decimalsOf` と Python `decimals_of` が同じ規則の 2 実装で、同値性の検定が 0 件（規約「同じコードを手書き複製するな」違反）。parity 検定を 1 本追加 |

### フォローアップ ISSUE 化（マージを止めない）

🟡-4（`ChartRenderer` 実体を丸ごと渡している＝ISP。同一ファイル `:370` に `createHostView` の正解形がある）／🟡-6b（台帳値の訂正時にテストの期待値 ≥9 箇所が赤になる＝「追加」は OCP を満たすが「訂正」は満たさない）／🔵-1（`NI225` は死んだデータ・docstring・分析スクリプトに残存。通過条件の文言を「配信 3 ページの実装から 0 件」へ精緻化）／🔵-2（replay 配信根への symlink 方針が不統一）／🔵-3（`marketdata` が A2 と A-MD の 2 アクターを内包＝TBD-D）／🔵-4（協働子未生成時の右クリック案内が事実と食い違う）／TBD-E（`priceInTable` の `digits ≠ 0` 対応。2 銘柄目を配信する時点で裁定）。

### 検定の重複についての判定（工程 5）

`test_symbol_spec_ledger.py` の `10 ** -digits <= tick`（不等式）は `digits == decimals_of(tick)`（等式）に論理的に包含されるが、**削除しない**。等式検定は**テスト内ヘルパ `decimals_of` の正しさに依存**し、ヘルパが退行すればタウトロジーとして緑になる。不等式検定はヘルパに依存しない下限保証であり、両者の関係は「重複」ではなく「強い検定＋独立ガード」。とくに 🟡-5（JS/Python の桁数規則が 2 実装で parity 検定なし）が未解消の状態で、唯一の独立ガードを外すのは順序が逆。

## 最終実 UI 検証（2026-08-20・HEAD 566118e）

是正後の実測。配信は `./unified_ui/serve.sh`（8000 ルータ）。**配信元ツリーの確認**は本ブランチにしか
存在しないファイル（`/live/js/domain/symbol_spec_generated.js`）が 200 で JP225 `tick:1.0` を返すことで行った
（8000 が 200 でも自分のコードとは限らないため・ISSUE-348 の教訓）。

| 確認項目 | 実測値 |
|---|---|
| ツールバー銘柄名 | `JP225`（`NI225` は DOM 0 件） |
| 現在値 | `65,453`（是正前 `65,721.051`） |
| バー情報 | `O 65,581 / H 65,591 / L 65,446 / C 65,453` |
| 指標レジェンド | `65,552.098` / `65,625.468` / `65,479.535`（**3 桁のまま＝対象外を巻き込んでいない**） |
| **コピーと画面の一致** | 画面 `O 68,866 H 68,896 L 68,594 C 68,604` ／ コピー `O 68,866\tH 68,896\tL 68,594\tC 68,604`＝**同一文字列**。コピー内の指標値は `MA 68,329.532` / `rsi 66.178` と 3 桁のまま |
| ピッカー | ゴースト `59,558` → 損切り欄 `59558`（**当初のバグの解消**） |
| 右クリック | 4 項目。損切り設定で `61853`（整数）。項目クリックでモーダルが自動で開く |
| 下段ペイン | トースト「価格チャート上で指定してください」 |
| 手入力 | `58700.4` → blur → `58700`／空欄は空欄のまま |
| 価格入力欄 | `step="1"`（`'any'` から台帳の `tick` へ） |
| リプレイ側 | 上記のうち銘柄名・右クリック・整数化を同様に確認（`61043`） |

### フェイルセーフの実 UI 実証（レビュー最大の未検証事項を解消）

工程 5 レビューは「フェイルセーフ分岐は本番配線から到達できないため未検証」と指摘した。
**リポジトリのファイルを一切変更せず、配信応答の `datasetRef` だけを経路で `zz_unknown_ref` に
差し替えて実測**した結果:

| 確認項目 | 実測値 |
|---|---|
| ツールバー | `銘柄未解決`（**空文字でも偽名でもない**） |
| トースト | `この銘柄の価格の刻みが不明なため、チャートからの価格指定を無効にしています` |
| `console.error` | `[position-sizing] 銘柄仕様が解決できません（datasetRef=zz_unknown_ref）: …` |
| 価格の書き戻し | **入らない**（モーダルが開かず値も入らない） |
| 現在値 | 空（データ自体が 400／404＝未知 ref をサーバが拒否。想定どおり） |

**無音で生値に落ちる経路は 1 つも無い**ことが、コード読解ではなく実ブラウザで確認できた。

### 最終ゲート

`indicator_ui/web` **2364** pass / 0 fail / skipped 0 ／ `tools/run_web_tests.sh` 全 5 スイート成功 ／
`pytest marketdata` 272 passed ／ `pytest simulator` 4678 passed ／ 参照実装 HTML と `chart_renderer.js`
の diff 0 行。
