...
def run():
    ...
    print("=" * 84)
    print(f"22EMA 下方乖離率の統計  JP225 日足  期間 "
    ...
    json.dump({"baseline_fwd20": round(base, 4), "stats": stats, "buckets": buckets},
              open(os.path.join(OUT, "deviation_22ema.json"), "w"),
              ensure_ascii=False, indent=2)
    _plot(dev_w, below, buckets, base)
    print(f"\n出力: {OUT}/deviation_22ema.json , deviation_22ema.png")
...
def _plot(dev_w, below, buckets, base):
    ...
    ax[0].set_title("22EMA deviation rate distribution (%)")
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "deviation_22ema.png"), dpi=110)
    plt.close(fig)
