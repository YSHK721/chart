...
層名/責務:
    ...
    DataFrame に正規化する。データ取得そのもの（元 VBA の ``inputData.getDataInCells``＝
    シート読込）は本層の責務外。

元 VBA の対応:
    ``iD.getDataInCells()`` → シートの OHLC を 2 次元配列に格納する処理。移植では CSV から。
...
_WORKSPACE_ROOT = Path(__file__).resolve().parents[3]  # src→price_range_power→indigators→repo 根
