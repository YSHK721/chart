"""デモ: 合成 OHLC に PRO!fitRSI を当て matplotlib PNG を生成する。

実行: python demo.py
matplotlib 未導入環境ではテストに影響しない設計（plot は __init__ から除外、本 demo
は実行時にのみ src.plot を import する）。RSI は volume を必要としないため、合成データ
は OHLC のみで構成する。
...
def main() -> None:
    from src.plot import plot_rsi  # 実行時にのみ matplotlib を import

    rng = np.random.default_rng(6)
    n = 300
    # トレンド転換を含む合成価格（上昇→レンジ→下降）で RSI の起伏を見せる。
    t = np.linspace(0, 9, n)
