...
`InpRSIPeriod=6`）を別ウィンドウ（`[0,100]`）の**RSI 線**（DRAW_LINE, clrLime）と
**EMA 平滑線**（`InpMAPeriod=5`, DRAW_LINE, clrLime）で可視化し、**生 RSI 系列**全体
...
|---|---|---|
| `src/core.py` | 純粋計算（numpy のみ） | `iRSI`（権威 Wilder, RSI 本線）/ `iMAOnArray(MODE_EMA)`（共有再利用）/ `iStdDevOnArray`＋`iMAOnArray(MODE_SMA)`（生 RSI に σ 7 水準）/ Apply→適用価格 写像 / indicator_minimum/maximum |
| `src/rsi.py` | 成果物層（DataFrame 整形） | `ExtRSIBuffer` / `ExtMABuffer`（DRAW_LINE）書き込み / StDevA1..A6 ＋ 50 水準 |
| `src/loader.py` | 入力アダプタ（CSV → OHLC） | `OnCalculate` 引数 open/high/low/close[] の供給（**volume 不要**） |
| `src/plot.py` | 出力アダプタ（matplotlib / PNG） | separate window [0,100] + DRAW_LINE×2(clrLime) + σ 水準線 7 本 + 短名（Apply 依存） |
...
out = build_rsi(df, rsi_period=6, apply=5, ma_period=5)   # rsi / rsi_ma 2 列（warm-up は 0）
levels = rsi_levels(df, rsi_period=6, apply=5, ma_period=5)  # p1/p2/p3/m1/m2/m3/mid50(=50)
```
...
plot_rsi(df, "out.png", rsi_period=6, apply=5, ma_period=5)  # 下段に RSI/EMA 線＋σ 水準線7本
```
...
add_rsi(sub, df, rsi_period=6, apply=5, ma_period=5)  # ライン2本(rsi/rsi_ma)＋水準線7本
```
...
2. **EMA 平滑**: RSI 系列（warm-up 0 込み）を**共有 `moving_averages` の EMA** で平滑化
   （in-package 再実装しない）。
3. **σ 7 水準**: **生 RSI 系列**全体（warm-up の 0 込み）の `平均 ±{1,2,3}×母標準偏差`
   ＋ 中央線 50。`iStdDevOnArray(ExtRSIBuffer)` / `iMAOnArray(ExtRSIBuffer, MODE_SMA)`
   相当（**EMA 後の ma ではなく生 RSI に掛ける**）。
4. **subwindow y 範囲**: `[0,100]`（元 indicator_minimum 0 / indicator_maximum 100）。
5. **短名**: "RSI-{適用価格名} ({period})"（Apply 依存。例 Apply=5 → "RSI-Typical price (6)"）。
...
- **σ 統計は生 RSI 系列に掛ける**: 元コードの `iStdDevOnArray`/`iMAOnArray` 引数が
  `ExtRSIBuffer`（EMA 後の ExtMABuffer ではない）。**warm-up の 0 も除外せず全系列で算出**。
- これらはいずれも**原挙動の 1:1 再現**であり "改善" しない（SPEC §9）。
- **iRSI は権威 Wilder（MetaQuotes 公式 `RSI.mq5`）準拠**。**EMA・適用価格は共有層を
  再利用**（重複排除）。**iRSI と σ 統計は in-package**（バッチ後 common 集約予定）。
