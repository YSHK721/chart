...
責務:
    ...
    high/low[] の供給）は本層の責務外。具体描画ライブラリ・pandas 以外の依存を内側
    （core/成果物層）へ侵入させない（依存内向き・PORTING_GUIDE §2）。
...
_WORKSPACE_ROOT = Path(__file__).resolve().parents[3]  # src→profit_hlband→indigators→repo 根
...
def load_ohlc_csv(
    ...
    **read_csv_kwargs,
    ...
    Raises:
        ...
        ValueError: 読み込んだ CSV が空（0 行）の場合。
    """
