"""デモ: 合成 OHLCV に PRO!fitMFI を当て matplotlib PNG を生成する。

実行: python demo.py
matplotlib 未導入環境ではテストに影響しない設計（plot は __init__ から除外、本 demo
は実行時にのみ src.plot を import する）。MFI は volume を必須とするため、合成データ
にも volume 列を含める。
...
def main() -> None:
    from src.plot import plot_mfi  # 実行時にのみ matplotlib を import

    rng = np.random.default_rng(14)
    n = 300
    # トレンド転換を含む合成価格（上昇→レンジ→下降）で MFI の起伏を見せる。
    t = np.linspace(0, 9, n)
    ...
    out = plot_mfi(
        df,
        out_path=str(Path(__file__).parent / "profit_mfi_demo.png"),
        mfi_period=14,
        ma_period=5,
        title="PRO!fitMFI (demo / synthetic OHLCV)",
    )
    print("saved:", out)
