"""デモ: 合成 OHLC に PRO!fitRSIMACD を当て matplotlib PNG を生成する。

実行: python demo.py
matplotlib 未導入環境ではテストに影響しない設計（plot は __init__ から除外、本 demo
は実行時にのみ src.plot を import する）。RSIMACD は iRSI を価格 Typical に当てるため
出来高（volume）は不要＝OHLC のみで成立する（合成データにも volume は含めない）。
...
def main() -> None:
    from src.plot import plot_rsimacd  # 実行時にのみ matplotlib を import
    ...
    out = plot_rsimacd(
        df,
        out_path=str(Path(__file__).parent / "profit_rsi_macd_demo.png"),
        rsi_period=13,
        fast=4,
        slow=8,
        signal=4,
        title="PRO!fitRSIMACD (demo / synthetic OHLC)",
    )
    print("saved:", out)
