...
責務:
    ...
    に保つ。元 MQL は別ウィンドウ（MACD 型・[0,100] 制約なし）のヒストグラム 1 本
    （MacdHistogram, DRAW_HISTOGRAM, C'133,219,24'）＋ ライン 2 本（RSIMACD ＝
    ...
    水平線 7 本を追加する。RSIMACD は価格 Typical 起点（iRSI）のため出来高（volume）
    は不要＝OHLC のみで成立する。warm-up（i<rsi_period）は元 iRSI 既定どおり 0 起点で
    描画される（NaN は発生しない）。

元 MQL 対応:
    ``SetIndexStyle(0, DRAW_HISTOGRAM)``（MacdHistogramBuffer, indicator_color1
    C'133,219,24'）/ ``SetIndexStyle(1, DRAW_LINE)``（MacdBuffer, label "RSIMACD",
...
依存（PORTING_GUIDE §8）:
    標準: __future__, typing / 外部: numpy, pandas / プロジェクト内: rsimacd, core
"""
...
# ライン名（元 SetIndexLabel: MacdBuffer="RSIMACD", SignalBuffer="Signal"）。
# 値列名はライン名と完全一致させる（PORTING_GUIDE §5）。
MACD_LINE_NAME = "RSIMACD"
...
def add_rsimacd(
    ...
    *,
    ...
    """chart に RSIMACD のヒストグラム 1 本・ライン 2 本・σ 水準線 7 本を追加する。
    ...
    Args:
        ...
        df: OHLC DataFrame（high/low/close 必須・列名大小不問・**volume 不要**）。
        rsi_period: RSI 期間（既定 13。元 RSIperiod）。
    ...
    # ライン 2 本（RSIMACD / Signal, DRAW_LINE）。値列名はライン名と完全一致させる（§5）。
    line_specs = (
