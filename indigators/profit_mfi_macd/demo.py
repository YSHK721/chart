"""デモ: 合成 OHLCV に PRO!fitMFIMACD を当て matplotlib PNG を生成する。

実行: python demo.py
matplotlib 未導入環境ではテストに影響しない設計（plot は __init__ から除外、本 demo
は実行時にのみ src.plot を import する）。MFIMACD は iMFI を起点とするため volume を
必須とするので、合成データにも volume 列を含める。
...
def main() -> None:
    from src.plot import plot_mfimacd  # 実行時にのみ matplotlib を import
    ...
    out = plot_mfimacd(
        df,
        out_path=str(Path(__file__).parent / "profit_mfi_macd_demo.png"),
        mfi_period=13,
        fast=4,
        slow=8,
        signal=4,
        title="PRO!fitMFIMACD (demo / synthetic OHLCV)",
    )
    print("saved:", out)
