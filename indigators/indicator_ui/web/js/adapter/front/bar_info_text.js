...
//   1 行目: 日時（日付 + 時間。読み取り欄と同じ fmtTime＝format.js が単一情報源）
//   2 行目: 四本値（O/H/L/C。読み取り欄と同じラベルと整形 fmtValue）
//   3 行目: 当日 MP（POC/VA。読み取り欄が出しているときと同じ条件＝DTO に載っているときだけ）
//   以降  : 指標 1 件 1 行（ペイン別凡例と同じ並び＝ペイン順・適用順）。
//           行は「凡例のラベル」＋系列ごとに「系列名 値」。凡例は系列名をツールチップに隠すが、
//           貼り付け先では名前が無いとどの系列か判別できないため必ず添える。
 ...
 * @param {Map|null} labels     instanceId → 凡例ラベル（PaneLegendView.labels()）。未指定は instanceId 表記。
 * @returns {string}            コピーする文字列。材料が何も無ければ空文字（＝呼び出し側はコピーしない）。
 */
export function formatBarInfoText(info, labels = null) {
  if (!info) {
    return '';
  }
  const lines = [];
