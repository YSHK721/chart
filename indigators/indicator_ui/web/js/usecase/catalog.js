...
// --- cvfe（条件付ボラティリティ予測 σ̂・別 pane オシレータ）--------------
// 正本仕様 indigators/cvfe/CVFE_spec_v1.0.md。次バーの条件付ボラティリティ σ̂_{t+1}
//   （対数価格の標準偏差）を HAR-CJ-L で 1 期先予測し、ギャップ分散を加算して合成する
//   （§4.5〜§4.8）。確定バー不変（非リペイント・切詰め不変をテストで固定）。
//   実バインディング add_cvfe（indigators/cvfe/src/lwc_chart.py）。
//   UI 計算経路が渡せるのは OHLC だけでティック列が無いため、仕様 §4.1-6 の
//   quality_gate="FAIL" 行が定める縮退（measure_id="PARK"）で算出する。精度は
//   仕様 §7-6 のとおり低下する（附録 A: Var(ln σ̂) は PARK 0.086 / RV 288 本 0.0017）。
//   表示は % 換算（σ̂ × 100）。系列名: cvfe / cvfe_oc / cvfe_co。
const CVFE = new IndicatorDef({
  ...
  params: [
    ...
    param('n_har', ParamType.INT, 500, [{ kind: ConstraintKind.MIN_VALUE, operands: ['n_har', 500], messageKey: 'err.n_har' }], null, {
      ...
      tooltip: 'HAR-CJ-L の学習に用いるバー本数（仕様 §3.1・下限 500）。先頭 n_har + 22 本は warm-up として σ̂ を出力しない。',
    }),
    ...
    param('lam_gap', ParamType.FLOAT, 0.97, [
      ...
      tooltip: 'ギャップ分散 EWMA の減衰係数 λ（仕様 §4.7-3）。大きいほど過去のギャップを長く引きずる。',
    }),
    ...
    // 成分 2 本（場中・ギャップ）の表示切替。
    param('show_components', ParamType.BOOL, true, [], null, {
      group: 'group.calc', order: 4, label: '成分を表示',
      tooltip: '場中成分 σ̂_OC（§4.6）とギャップ成分 σ̂_CO（§4.7）を点線で併せて描く。',
    }),
    param('color', ParamType.COLOR, 'rgba(233, 30, 99, 1)', [], null, { group: 'group.style', order: 1 }),
  ],
  // 系列: σ̂（実線）＋ 場中成分・ギャップ成分（点線）。いずれも別 pane。
  series: [
    new SeriesDef({ kind: SeriesKind.LINE, sourceColumn: 'cvfe', seriesName: 'cvfe', dynamic: false, barStyleEditable: true }),
    new SeriesDef({ kind: SeriesKind.LINE, sourceColumn: 'cvfe_oc', seriesName: 'cvfe_oc', dynamic: false }),
    new SeriesDef({ kind: SeriesKind.LINE, sourceColumn: 'cvfe_co', seriesName: 'cvfe_co', dynamic: false }),
  ],
