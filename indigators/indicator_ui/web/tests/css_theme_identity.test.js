...
//   2026-09-05 `.prop-field-desc`（ⓘ クリックで開く説明ブロック・ISSUE-494）を新設。
//     宣言 3 件が増え `#787b86` 22→23（--ct-uiTextWeak）・`#131722` 9→10（--ct-uiSurface）・
//     `#2962ff` 22→23（--ct-uiAccent）。総数 301→304・色数 47 は不変（新色を導入しない）。
const EXPECTED_APP = [
  ['#2a2e39', 60], ['#ffffff', 37], ['#d1d4dc', 36], ['#2962ff', 23], ['#787b86', 23],
  ['#9aa0ad', 14], ['#1e222d', 10], ['#131722', 10], ['#23272f', 8], ['#b2b5be', 8],
...
test('通過条件 1: app.css がテーマ未設定で解決する色は接続前と全数一致（301 件 / 47 色）', () => {
  ...
  assert.equal(total, 304, '実測総数（2026-09-05 の .prop-field-desc 新設ぶん 3 件を加算）');
