...
test('UC-01 listForView: empty filter returns all 24', () => {
...
test('UC-01 listForView: filters by tab', () => {
  // 23 指標（既存 19 + btlm_trail + btlm_trail_marod + ma_marod + cvfe）は tab=indicator、market_profile のみ tab=profile。strategy は 0 件。
  assert.equal(listForView({ tab: 'strategy' }).length, 0);
  assert.equal(listForView({ tab: 'indicator' }).length, 23);
