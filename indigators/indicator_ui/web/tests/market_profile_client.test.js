...
// day（単日拡大ビューの左70%パス）: day 指定時 &day=<date> を付与（省略時は付けない・後方互換）。
//   本タスク: 左70%=その日のティック推移。actor が focus 時に day 付きで再取得する。
test('buildMarketProfileUrl appends &day=<date> when day is provided', () => {
  const url = buildMarketProfileUrl({ datasetRef: 'jp225_tick', day: '2024-01-01' });
  assert.ok(url.includes('&day=2024-01-01'));
});

test('buildMarketProfileUrl omits &day when not provided/null (後方互換)', () => {
  assert.ok(!buildMarketProfileUrl({ datasetRef: 'sample' }).includes('day='));
  assert.ok(!buildMarketProfileUrl({ datasetRef: 'sample', day: null }).includes('day='));
});

// parseProfileResponse: トップレベル day_path[] を profile へ素通し（actor が primitive へ渡す）。
test('parseProfileResponse passes through top-level day_path into profile', () => {
  const payload = {
    ok: true,
    profile: {
      bins: [{ price: 100, tpo: 1, norm: 1 }], poc: 100, va_low: 100, va_high: 100,
  ...
  const out = parseProfileResponse(payload);
  assert.equal(out.day_path.length, 2);
  assert.equal(out.day_path[0].t, 1704067200);
  assert.equal(out.day_path[1].p, 1001.0);
});

test('parseProfileResponse omits day_path key when absent (後方互換)', () => {
  const payload = {
    ok: true,
    profile: {
      bins: [{ price: 100, tpo: 1, norm: 1 }], poc: 100, va_low: 100, va_high: 100,
