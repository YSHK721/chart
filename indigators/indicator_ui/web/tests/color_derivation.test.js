...
test('TC-CD06 level ← text と surface: 対地 CR が目標 5.249 になる点（混合比ではない・ISSUE-345）', () => {
  // Arrange: 目標は**混合比ではなく対地コントラスト比**で持つ（ISSUE-345 の抜本是正）。混合比は
...
test('TC-CD07 muted ← level と surface: 対地 CR が目標 3.3 になる点（W-C2 に余裕を持たせる）', () => {
  ...
  //   （ISSUE-345 が muted について指摘した「余裕の薄さ」そのもの）。10% の余裕を明示的に置く。
  const declared = { surface: CURRENT.surface };
  // Act
  const out = expandRoleColors(declared);
  // Assert
...
// =========================================================================
// ISSUE-345 の是正: 地に従属する導出（level / muted）は混合比ではなく対地 CR の目標で持つ
//
...
// 検定に使う代表的な地。表（ISSUE-345）の 6 種に、伸びしろの狭い高彩度色を足す。
...
test('TC-CD30 恒等: 現行の暗い地と濃紺では診断が 0 件のまま（見た目の回帰が無い）', () => {
  // Arrange: ISSUE-345 の是正が、現行値と実際に使われる暗いテーマの領域を動かしていないこと。
