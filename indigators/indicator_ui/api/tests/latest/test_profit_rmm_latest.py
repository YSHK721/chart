"""Stage B 検証: profit_rmm を Latest 増分計算フレームワークへ分類＋一致検証。
...
profit_rmm の分類（実コード接地）:
  * バインディング: call_binding._TABLE[("profit_rmm","default")] = add_rmm（histogram）、
    catalog def.series = [PF_HIST('rmm_lc'), PF_HLINE('profit_rmm')]。単一 variant=default。
  * 系列 kind: {histogram, horizontal_line} → horizontal_line を含むため frontend routing は "full"。
  * archetype: core.compute_rmm は iRSI/iWPR/iMFI と EMA（exponential_ma_on_buffer の
    buf[i-1] 漸化）を合成し、各オシレーターのスパンを因果ローリング窓（window=DEFAULT_WINDOW=120）
    の母σで標準化して funLevelCount を採点・合算する（histogram=level_count 時系列）。出力には
    EMA 等の先頭シードからの再帰依存があり df.tail で開始点を変えると末尾値がドリフトしうるため、
    先頭からの full 計算が必須。さらに σ6 水準線（compute_rmm_levels＝全系列の母平均±kσ）を
    horizontal_line（価格軸分布）として出力する。latest_meta に未登録 → 安全既定
    LatestMeta("recurrence", None, 1)（full＋K=1）が適用される。full 経路のため latest の
    histogram 末尾 K 点は full の末尾 K 点と float 完全一致する。
...
_COMPUTE_ID = "profit_rmm"
_VARIANTS = ("default",)
_TRIMMABLE = ("line", "histogram")


def _ohlcv(n: int = 200) -> pd.DataFrame:
    """昇順 OHLCV（合成・osc_period=6 / ma_period=6 / window=120 を満たす十分な本数）。"""
...
def _params() -> dict:
    """catalog 既定（osc_period=6 / ma_period=6 / window=120）。"""
    return {"osc_period": 6, "ma_period": 6, "window": 120}


def test_latest_meta_profit_rmm_safe_default_recurrence_full_k1():
...
def test_latest_runs_without_error_and_returns_series():
    ...
    for variant in _VARIANTS:
        ...
        # catalog 由来: histogram（rmm_lc）＋ horizontal_line（σ6 水準）。
