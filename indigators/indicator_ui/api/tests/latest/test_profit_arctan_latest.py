"""Stage B 検証: profit_arctan を Latest 増分計算フレームワークへ分類＋一致検証。
...
profit_arctan の分類（実コード接地）:
  * バインディング: call_binding._TABLE[("profit_arctan","default")] = add_arctan（histogram）、
    catalog def.series = [PF_HIST('arctan_lc'), PF_HLINE('profit_arctan')]。単一 variant=default。
  * 系列 kind: {histogram, horizontal_line} → horizontal_line を含むため frontend routing は "full"。
  * archetype: core.compute_level_count は ps_level_count の標準化窓（window）で σ 距離を算出し、
    出力は σ12 水準線（horizontal_line＝価格軸分布）＋クランプ済 level_count（histogram 時系列）。
    latest_meta に未登録 → 安全既定 LatestMeta("recurrence", None, 1)（full＋K=1）が適用される。
    full 経路のため latest の histogram 末尾 K 点は full の末尾 K 点と float 完全一致する。
...
_COMPUTE_ID = "profit_arctan"
_VARIANTS = ("default",)
_TRIMMABLE = ("line", "histogram")


def _ohlcv(n: int = 200) -> pd.DataFrame:
    """昇順 OHLCV（合成・period=6 / window=120 を満たす十分な本数）。"""
...
def _params() -> dict:
    """catalog 既定（period=6 / ma_method=1 / bar_width=0.1 / window=120）。"""
    return {"period": 6, "ma_method": 1, "bar_width": 0.1, "window": 120}


def test_latest_meta_profit_arctan_safe_default_recurrence_full_k1():
...
def test_latest_runs_without_error_and_returns_series():
    ...
    for variant in _VARIANTS:
        ...
        # catalog 由来: histogram（arctan_lc）＋ horizontal_line（σ12 水準）。
