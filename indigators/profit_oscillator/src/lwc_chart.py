...
差分:
    profit_arctan/src/lwc_chart.py から踏襲（同じ PS レベルカウント・σ12）。ただし
    入力が OHLCV（iMFI が volume を要する）であり、計算は成果物層 ``build_oscillator`` /
    ``oscillator_levels`` に委譲する（OHLCV 列抽出・必須列欠落の KeyError は成果物層が担う）。

依存:
    標準: __future__, typing / 外部: pandas / プロジェクト内: oscillator
"""
...
def add_oscillator(
    ...
    *,
    ...
    # 値列名はヒストグラム名と完全一致させる（PORTING_GUIDE §6）。NaN は描画側で除外。
