...
def test_dependency_is_numpy_only():
    ...
    適用範囲: 仕様 §6 の「依存: numpy のみ」は **計算エンジン**（§3.1〜§5 が定める
    入出力と処理）への制約である。出力アダプタ ``lwc_chart.py`` は仕様の対象外で、
    チャート UI へ系列を供給する層として既存指標と同じ pandas 前提の規約
    （``common_view.lwc_adapter``）に従う。よって当該 1 ファイルのみ除外する。
    """
    import ast

    # 仕様 §6 の対象外（出力アダプタ）。除外はこの 1 件に限る。
    adapter_only = {"lwc_chart.py"}
    ...
    files = [f for f in sorted(src_dir.glob("*.py")) if f.name not in adapter_only]
    assert files, "src が空では検証にならない"
    assert (src_dir / "lwc_chart.py").exists(), "除外対象が存在しない＝除外リストが陳腐化している"
    seen_numpy = False
    for path in files:
