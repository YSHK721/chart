...
バージョン：1.0（+ 2026-07-30 裁定改訂 12 件）/ 作成日：2026-07-29
...
| 2026-07-30 | `DEGRADED` 閾値を `S > 0.45` へ引き下げ | §4.1-6 | ISSUE-207。`S(ω/σ=0.5) = 0.4877` が閾値 0.50 に構造的に届かず §9 段階 2 を満たせなかった |
| 2026-07-30 | OHLC しか渡せない経路の縮退を正式に規定 | §3.1 | ISSUE-218。UI 表示は概形把握が目的であり効率差を許容する |

**本仕様の既知欠陥はすべて裁定済み**（§9 段階 1・2 の `xfail` は 0 件）。
...
本仕様の入力は `ticks` とする。**OHLC しか渡せない経路**（`indicator_ui` のチャート表示。
`call_binding._TABLE` → `add_*(chart, df, ...)` は OHLC の DataFrame のみを運ぶ）では §4.1 の
気配品質診断（`RV̄(Δ)` / `ω̂²` / `freeze_ratio` / `S`）を実行できないため、§4.1-6 の
「高頻度データを使用しない」行に沿って `quality_gate = "FAIL"` / `measure_id = "PARK"` へ
縮退する（**ISSUE-218 改訂**。実装は `indigators/cvfe/src/ohlc.py`）。

精度を要する消費者（CEB v1.1 への `sigma_hat` 供給）は**バッチ経路 `compute_cvfe`** を用いること。
UI 表示は概形把握を目的とし、附録 A の効率差（`Var(ln σ̂)` が `PARK` 0.08575 対 `RV` 0.00174 ＝
**49.3 倍**）を許容する。UI 経路にティック供給口を新設する案は、`call_binding` の共有シグネチャ
変更を伴い 21 指標パッケージ全体へ波及するため採らない。
...
6. 判定（この順に評価し、最初に成立した分岐を採用する）。
   ...
   **ISSUE-207 改訂**：`DEGRADED` 閾値は v1.0 の `S > 0.50` から **`S > 0.45`** へ引き下げた。
   `S = (2 − 1/30)r² / (1 + r²/30)`（`r = ω/σ`）であるから `r = 0.5` の理論値は `0.4877`、
   実測は `0.4718` であり、いずれも `0.50` に届かず `PASS` と判定されていた。これは §9 段階 2 の
   「`ω/σ ≥ 0.5` のとき `"DEGRADED"` 以上を返す」という要求を**構造的に満たさない**。閾値 `0.50` は
   本仕様が根拠を示していない固定値であり `S(r=0.5)` を計算せずに置かれた可能性が高い一方、
   §9 の要求は設計意図（ノイズが信号の半分に達したら警告する）である。意図を正とし、理論値
   `0.4877` を下回る丸い値として `0.45` を採る。

   | 条件 | `quality_gate` | 帰結 |
   |------|---------------|------|
   | `freeze_ratio > freeze_thresh` | `"FAIL"` | 高頻度データを使用しない。`measure_id = "PARK"` |
   | `S > 0.45` | `"DEGRADED"` | ノイズ大。`measure_id = "TSRV"`、`delta_star_sec = 300`（**ISSUE-215 改訂**。§3.2 が値を要求するのに v1.0 は規定していなかった）。§4.4 のジャンプ分離は**実行しない** |
   | `0.10 < S ≤ 0.45` | `"PASS"` | `measure_id = "RV"`、`delta_star_sec = 300` |
...
- [ ] マイクロストラクチャノイズ（`ω/σ = 0.1, 0.5, 1.0`）を注入した条件で、
      ...
      **ISSUE-207 改訂**：`ω/σ = 0.5` での `S` は理論値 `0.4877`・実測 `0.4718` であり、v1.0 の
      閾値 `S > 0.50` には構造的に届かなかった。§4.1-6 の閾値を `S > 0.45` へ引き下げて本項を満たす。
