...
責務:
    ...
    PRO!fitRMM は iMFI を内包し出来高（volume）を必須とするため、先例 profit_mfi の
    ``load_ohlcv_csv`` を踏襲し volume を必須列とする。

元 MQL4 対応:
    ``PRO!fitRMM.mq4`` の ``OnCalculate`` 引数 high/low/close/volume[]（既定の MT4
    チャート出来高）相当のデータ供給を CSV から行う。volume の値（tick / 実出来高）は
    CSV の列定義に従いそのまま採用する（bit-exact は CSV 列定義依存・SPEC §9）。
...
_WORKSPACE_ROOT = Path(__file__).resolve().parents[3]  # src→profit_rmm→indigators→repo 根
...
# RMM 必須列（volume を含む）。open は RMM 計算には不使用だが OHLCV 整合のため必須化。
