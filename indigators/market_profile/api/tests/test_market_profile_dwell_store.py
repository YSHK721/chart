"""DwellRollupStore（adapter/compute/market_profile_dwell_store.py）単体検証。
...
（本体 market_profile_dwell は module 変数 `_CACHE_ROOT`/`_CACHE_VERSION`/`day_parquet_files` を
provider に束ねて渡す。既存テストの `_CACHE_ROOT` tmp 注入経路は本体側 shim で温存する）。
"""
...
class TestLoadFailSafe:
    ...
    def test_version_mismatch_returns_cache_miss(self, tmp_path):
        ...
        path = s.cache_path("JP225", _DAY0)
        s.save_day_rollup(path, {"kmin": 5, "dwell": np.array([1.0]), "cnt": np.array([1.0])})
    ...
    def test_grid_mismatch_returns_cache_miss(self, tmp_path):
        ...
        s_save.save_day_rollup(path, {"kmin": 5, "dwell": np.array([1.0]), "cnt": np.array([1.0])})
