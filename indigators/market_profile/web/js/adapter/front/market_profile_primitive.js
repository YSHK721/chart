...
const C_VA_LINE = 'rgba(154, 164, 178, 0.3)';
...
export class MarketProfileHistogramPrimitive extends PairPrimitiveBase {
  ...
  _drawTfPeriods(ctx, scope, toY) {
    ...
    for (let i = 0; i < cols.length; i += 1) {
      ...
      // ISSUE-084 改（依頼者指示 2026-07-15）: VA 幅は**境界ライン**（VAH/VAL の横線・灰）で表現する。
      //   旧「VA 外減光」は zp のように levels が sparse（z>0 のみ）で VA が可視レベル全域を覆う列では
      //   減光対象が無く判別不能だった（実測: 1D zp で VA外レベル 0 の列が多数）。ラインは列ごとに常に
      //   描ける。va 欠損列（旧応答・空日）は描かない（後方互換）。
      if (c.va_low != null && c.va_high != null) {
        const yH = toY(c.va_high);
        const yL = toY(c.va_low);
        ctx.fillStyle = C_VA_LINE;
        if (yH != null) ctx.fillRect(left + 1, yH - 0.5, tileW - 2, 1);
        if (yL != null) ctx.fillRect(left + 1, yL - 0.5, tileW - 2, 1);
      }
    }
    ctx.restore();
  }
