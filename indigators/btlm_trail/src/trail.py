...
class TrailResult:
    ...
    Attributes:
        ...
        bands: {(q_low, q_high): (low[], high[])}。分位ペアごとのバンド。
...
def _validate_pairs(quantile_pairs) -> list[tuple[float, float]]:
    """分位ペアを検証する（0 < q_low < q_high < 1）。空・重複順序違反は ValueError。"""
    if not quantile_pairs:
        raise ValueError("分位ペアが空です。")
    out: list[tuple[float, float]] = []
    for pair in quantile_pairs:
        ql, qh = float(pair[0]), float(pair[1])
        if not (0.0 < ql < qh < 1.0):
            raise ValueError(
                f"分位ペアは 0 < q_low < q_high < 1 が必要です: {pair}"
            )
        out.append((ql, qh))
    return out
...
def build_btlm_trail(
    ...
    *,
    ...
    """
    pairs = _validate_pairs(quantile_pairs)
    ...
    bands: dict[tuple[float, float], tuple[np.ndarray, np.ndarray]] = {}
    if method == "ols":
        for ql, qh in pairs:
            low = mean + norm_ppf(ql) * pred_sd
            high = mean + norm_ppf(qh) * pred_sd
            bands[(ql, qh)] = (low, high)
    else:
        ...
        for ql, qh in pairs:
            ...
            bands[(ql, qh)] = (mean * (1.0 + emp_lo), mean * (1.0 + emp_hi))

    return TrailResult(mean=mean, beta=beta, sigma=sigma, bands=bands, band_method=method)
...
def realized_coverage_latest(
    close: np.ndarray, band: tuple[np.ndarray, np.ndarray], n_cov: int = DEFAULT_N_COV
) -> float:
