...
//   指標更新は「要求フラグ＋クロック」で駆動する。イベント（バー確定）はフラグを
...
//
// ISSUE-250 Phase 1（足内要求の廃止）: 本スケジューラが扱うのは**バー確定 full 再計算のみ**。
//   tick 粒度の末尾値は /live_ticks へ同梱され、LiveTickPlayer が updateLastCandle と同一同期
//   ブロックで描く（HTTP 往復なし）。旧 forming 要求は 1 往復 = 1 回の coalesce を伴うため、
//   「指標更新回数 == ローソク更新回数」を構成上成立させられなかった（要求を残すと、往復由来の
//   更新が tick と無関係な回数で混ざる）。よって要求経路ごと外す。
//
// 依存注入（DIP・テスト容易性）:
...
export class UpdateScheduler {
  constructor({ runFull, isBlocked = () => false, now = Date.now }) {
    ...
    (async () => {
      ...
      try {
        ...
        if (typeof console !== 'undefined' && console.warn) {
          console.warn('full 再計算失敗（次クロックで再試行）:', err && err.message);
        }
      } finally {
