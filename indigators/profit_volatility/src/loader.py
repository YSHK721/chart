...
差分:
    profit_arctan/src/loader.py から踏襲。iVOLATILITY は 7 適用価格に ``open`` を含む
    （X/Y digit の 1=Open）ため、必須列は open/high/low/close の 4 列（volume 不要）。
...
_WORKSPACE_ROOT = Path(__file__).resolve().parents[3]  # src→profit_volatility→indigators→repo 根
