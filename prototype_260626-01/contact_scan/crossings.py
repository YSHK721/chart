"""crossings — レベルに対する系列の sign 変化（クロス）検出（純・新規）。

接点（クロス）検出の汎用ヘルパは既存に無いため新規。``detect_crossings`` は時系列
``series = [(t, val), ...]`` を走査し、``level`` に対する符号（+1=上 / -1=下 / 0=タッチ）の
非ゼロ符号が反転した位置を 1 接点イベントとして列挙する。
