...
export function ladderRow(overrides = {}) {
  return {
    ...
    // MP 列（依頼者承認 2026-09-06）: 1m×2000 本プロファイルの TPO 密度 norm（0..1）。
...
export function oscCell(overrides = {}) {
  return {
    ...
    timeframe: '1m',
    value: 0.8,
    thresholds: [],    // 指数の閾値水準（降順・{level, value}・依頼者指示 2026-09-05「q 水準を表示」）
    p: 0.31,
    tail_unscaled: false,
    reach: null,
    unavailable_reason: null,
    level_prices: { q_high: null, q_low: null, ext_hi: null, ext_lo: null },   // 各側は {price, level} | null
    history: [],   // 直近の確定区間の読み（古い順・各要素 {value, p, tail_unscaled}）
    cumulative: false,   // §5.3.3 の積み上がる量か（ミニ描画の形: 棒 / ライン）
