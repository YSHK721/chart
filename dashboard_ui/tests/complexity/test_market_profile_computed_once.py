...
なぜ状態検証と別に要るか:
    プロファイル（1m×2000 本の TPO 畳み込み）は確定足だけから決まるので、epoch の中では
...
数える面（**面ごとに**数える）:
    畳み込み（`ProfileSpy`）だけでなく、ビン数を決める価格レンジ面（`RangeSpy`）も数える。
    後者は窓の全 2000 本を走査する第 2 の計算面であり、畳み込みを通らない。片方だけを
    計装すると、レンジだけが毎要求走るようになっても勘定は 0 のまま・出力も正しいままで、
    やはり原理的に落ちない（同型の取り逃しを面の数だけ作らない）。
...
固定するもの（**回数そのものは焼き込まない**）:
    ...
    - 不変量 D（鮮度）: 確定 1m 足が進んだら作り直し、以後の繰り返しでは追加 0 に戻る。
    - 不変量 F（時計非依存）: 供給の末尾 1 本（形成中相当）が動いても、**表示足がどれだけ
      粗くても**追加発行は 0（両方の面で）。窓を時計で決めると、粗い表示足では形成中足が窓へ残り
      毎ティック再計算になる（出力は正しいままなので状態検証では落ちない）。
"""
...
START = 1_785_542_400
MINUTE = 60
#: 粗い表示足の刻み（シートの `now_unix` は**表示足**の末尾 time である）。
DISPLAY_STEP = 3_600
...
class Material:
    """1m の素材（確定足 ＋ 末尾の形成中足）。周期を進めたり末尾だけを動かしたりできる。"""

    def __init__(self, *, rows: int) -> None:
        self.times = [START + index * MINUTE for index in range(rows)]
        self.closes = [120.0 + index * 0.1 for index in range(rows)]

    def advance_epoch(self) -> None:
        """周期を 1 つ進める（新しい確定足が現れる＝確定素材が変わる）。"""
        self.times.append(self.times[-1] + MINUTE)
        self.closes.append(self.closes[-1] + 0.1)

    def move_forming_bar(self, close: float) -> None:
        """供給の末尾（形成中相当）だけを動かす（周期は進まない＝確定素材は変わらない）。

        `bars()` は終値から高安も作るので、これ 1 つで close / high / low が同時に動く。
        """
...
class BarPortSpy:
    ...
    `forming_bar` は「素材の末尾が現在の周期を覆っていれば」返す面である（実装は
    `dashboard_ui/adapter/gateway/indicator_ui_compute_gateway.py`）。ここでも同じく
    時計と突き合わせる——粗い表示足の `now_unix` では None になり、時計に頼った窓は
    形成中足を確定足として飲み込む。
    """
    ...
    def forming_bar(self, *, dataset_ref, timeframe, now_unix):
        ...
        covered = supplied and int(supplied[-1].time) // MINUTE == int(now_unix) // MINUTE
        return supplied[-1] if covered else None
...
class RangeSpy:
    ...
    畳み込みとは**別の計算面**である: ビン数を決めるために窓の全
    `market_profile_gateway.MP_WINDOW_BARS` 本（運用では 2000 本）を走査して高安を取る。`ProfileSpy` はこの面を一切通らないので、
    レンジだけが毎要求走るようになっても畳み込みの勘定は 0 のままで、**出力も正しいまま**
    である（状態検証でも畳み込みの勘定でも原理的に落ちない）。面ごとに数える。

    実物へ委譲するので、差し替えても挙動・出力は変わらない。
    """
...
#: 差し替え前の実物。1 つの検定が 2 点固定のために複数回差し替えるので、**常にこれを包む**
#: ——そのとき居る面を包むと Spy が入れ子になり、外側の勘定は正しいまま内側の勘定だけが
#: 別の点の呼び出しを混ぜて読む（測る道具が測る対象を汚す）。
_REAL_PRICE_RANGE = market_profile_gateway._core_price_range


def install_range_spy(monkeypatch: "pytest.MonkeyPatch") -> RangeSpy:
    """価格レンジ面を Test Spy へ差し替える（実物へ委譲＝出力は不変）。"""
    spy = RangeSpy(_REAL_PRICE_RANGE)
...
def request(
    spy: ProfileSpy, material: Material, store: MaterialStore, *,
    rows: int = 3, now_unix: "int | None" = None,
):
    """要求 1 件ぶん（**口は要求ごとに組み直す**——共有するのは素材ストアだけ）。

    `now_unix` の既定は素材の末尾（＝表示足も 1m の場合）。粗い表示足を模すときだけ
    呼び出し側が別の時刻を渡す。
    """
    gateway = MarketProfileGateway(
        bar_port=BarPortSpy(material), store=store, compute=spy,
    )
    clock = material.times[-1] if now_unix is None else int(now_unix)
    return gateway.norms_at(dataset_ref=REF, prices=prices(rows), now_unix=clock)


# ------------------------------------------------------- 不変量 A: 無駄の不在
def test_repeating_the_same_request_issues_no_additional_profile(
    ...
    畳み込みと同じ勘定を**価格レンジ面**にも課す（`RangeSpy`）。ビン数を決める窓の走査は
    畳み込みとは別の面なので、`market_profile_gateway._bins_for` が持ち越しの factory の
    外へ出れば、畳み込みが
    1 回のままでもレンジだけが毎要求走る。出力は正しく、畳み込みの勘定も 0 のままなので、
    この面を数えなければ検出できない。
    """
    additional = {}
    additional_ranges = {}
    for repeats in (5, 20):
        ...
        spy = ProfileSpy()
        ranges = install_range_spy(monkeypatch)
        store = MaterialStore()
        request(spy, material, store)          # 1 回目（この epoch の素材を作る）
        warmed = (len(spy.calls), len(ranges.calls))
        for _ in range(repeats):
            request(spy, material, store)
        additional[repeats] = len(spy.calls) - warmed[0]
        additional_ranges[repeats] = len(ranges.calls) - warmed[1]

    assert additional[5] == 0
    assert additional[20] == 0
    assert additional_ranges[5] == 0
    assert additional_ranges[20] == 0


def test_a_moving_forming_bar_issues_no_additional_profile() -> None:
    """形成中足が動いただけでは作り直さない（更新粒度は 1m バー確定・§7 段 2）。"""
    material = Material(rows=90)
    spy = ProfileSpy()
    store = MaterialStore()
    request(spy, material, store)
...
def test_a_moving_tail_bar_issues_no_additional_profile_on_a_coarse_display_clock(
    monkeypatch: "pytest.MonkeyPatch",
) -> None:
    """**時計が粗くても** 0（オーダーの表明を時計の粗さ 2 点で固定する）。

    シートの `now_unix` は表示足の末尾 time である。表示足が 1m より粗いと、その時刻は
    供給末尾の 1m 足の周期に載らないので `forming_bar` は None を返す。窓を形成中判定で
    決めていると形成中の 1m 足が窓に残り、epoch がティックごとに動いて毎ティック
    再計算になる（8 分の 7 の表示足で発生する・出力は正しいままなので状態検証では
    原理的に落ちない）。窓は末尾 1 本を無条件に落とす＝時計に依存しない。

    ここでも**価格レンジ面**を併せて数える（`RangeSpy`）。時計の粗さで壊れるのは持ち越し
    そのものなので、壊れれば畳み込みも窓の走査も等しく毎ティック走る。面ごとに数えて
    おけば、どちらか一方だけが漏れた場合にも赤くなる。
    """
    additional = {}
    additional_ranges = {}
    for clock_step in (MINUTE, DISPLAY_STEP):
        material = Material(rows=90)
        spy = ProfileSpy()
        ranges = install_range_spy(monkeypatch)
        store = MaterialStore()
        clock = material.times[-1] // clock_step * clock_step
        request(spy, material, store, now_unix=clock)
        warmed = (len(spy.calls), len(ranges.calls))

        for tick in range(20):
            material.move_forming_bar(130.0 + tick)
            request(spy, material, store, now_unix=clock)
        additional[clock_step] = len(spy.calls) - warmed[0]
        additional_ranges[clock_step] = len(ranges.calls) - warmed[1]

    assert additional[MINUTE] == 0
    assert additional[DISPLAY_STEP] == 0
    assert additional_ranges[MINUTE] == 0
    assert additional_ranges[DISPLAY_STEP] == 0
...
# ------------------------------------------------------------ 不変量 D: 鮮度
def test_a_new_confirmed_minute_bar_is_rebuilt_once_and_then_settles() -> None:
...
def test_the_output_follows_the_new_confirmed_bar_instead_of_the_stale_profile() -> None:
    ...
    assert after != before
    assert spy.calls[-1][-1] == material.times[-2]   # 窓の末尾＝新しい確定足（末尾は落とす）
