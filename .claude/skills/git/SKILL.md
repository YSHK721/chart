...
- **手順**：
  ...
  2. `git diff --cached --stat` を読む
  3. **目的に属さないファイルが 1 件でもあれば `git restore --staged <path>` で外す**。外し終えるまで次へ進まない
  4. ローカルで lint / format / secret-scan / 静的解析 / テストを実行し合格を確認
  5. `type(scope): summary` 形式でメッセージ作成。本文に意図・影響を箇条書き、必要に応じ `Refs: #<ID>` を付記
  6. `git commit -m "..."` を実行
...
- **手順**：
  ...
  4. `git diff --cached --stat` を読み、**解決対象以外のファイルがあれば `git restore --staged <path>` で外す**
