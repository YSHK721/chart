...
| 一括ステージング禁止 | `git add -A` / `git add .` / `git add -u` を禁止する。パスを明示して `git add <path>`、または `git add -p` を用いる |
| ステージング内容の確認義務 | `git commit` の前に `git diff --cached --stat` を必ず読み、意図しないファイルが含まれていないことを確認する。確認結果を報告に含める |
| 環境依存物のコミット禁止 | symlink・venv・データ実体・ローカル設定をコミットしない。とくに**中身が絶対パスの symlink** は、別チェックアウトへ展開されると「自分自身を指す symlink」になり実体参照を失わせる（破壊的変更に該当） |
...
### step S-3: 原子的コミット実行
- **判定基準**：1 コミット = 1 目的を満たすか／関連ファイル・テスト・ドキュメントが整合しているか／メッセージが Conventional Commits 準拠かつ曖昧語を含まないか／**ステージされた全ファイルを列挙して読み、そのすべてが当該目的に属すると言えるか**
- **アンチパターン**：複数目的の混在コミット／関連テスト未更新／曖昧語のみのメッセージ／秘密情報の混入／**一括ステージング（`git add -A` / `git add .` / `git add -u`）**／**ステージング内容を確認せずコミット**
- **手順**：
  1. `git add -p` で粒度を調整、またはファイル単位で `git add <path>`。**一括ステージングは §4 で禁止**
  2. `git diff --cached --stat` を読み、**意図しないファイルが 1 件も含まれていない**ことを確認する（この確認は省略不可）
  3. ローカルで lint / format / secret-scan / 静的解析 / テストを実行し合格を確認
  4. `type(scope): summary` 形式でメッセージ作成。本文に意図・影響を箇条書き、必要に応じ `Refs: #<ID>` を付記
  5. `git commit -m "..."` を実行

> **なぜ手順 2 が省略不可か（2026-08-10 の実例・ISSUE-363）**
> worktree で core を起動するために張った symlink 2 本（中身は `/workspaces/app/...` の絶対パス）を
> `git add -A` が未追跡ファイルとして拾い、マージコミットへ混入した。本チェックアウトへ展開された
> 時点で「自分自身を指す symlink」になり、venv とデータの実体参照が失われてサーバが起動しなくなった。
> `git diff --cached --stat` に `lightweight-charts-python-main/.venv | 1 +` の 2 行が出ていたため、
> **手順 2 を実行していれば防げた**。git の操作としては「追加」だが、結果は既存データの破壊であり、
> 復元に人手を要した（破壊的変更に該当）。
> 環境依存物は `tools/setup_worktree.sh`（環境変数）で用意し、symlink を張らないこと。
> 詳細: `docs/git-worktree-workflow.md` §7。
...
### step S-6: コンフリクト解決
- **判定基準**：衝突箇所の意図的整合・テスト合格・差分再点検が完了しているか／**マージコミットに、どちらの親にも無いファイルが混入していないか**
- **アンチパターン**：衝突マーカー残置／片側のみ採用で意図無視／解決後のテスト省略／**衝突解決時に `git add -A` を使い、作業ディレクトリの未追跡ファイルを巻き込む**
- **手順**：
  1. `git status` で衝突ファイル特定
  2. 衝突ファイルを編集して解消
  3. `git add <resolved-files>` でステージング（**解決したファイルのみ**。一括ステージング禁止）
  4. `git diff --cached --stat` を読み、解決対象**以外**のファイルが入っていないことを確認する
  5. rebase 中は `git rebase --continue`、merge 中は `git commit -m "fix: resolve merge conflicts in <module> due to <reason>"`
  6. テスト再実行と差分再点検で動作確認

> **マージコミット特有の危険（2026-08-10 の実例・ISSUE-363）**
> マージコミットは**両方の親のどちらにも無い変更を含めることができる**。衝突解決の途中で
> `git add -A` を打つと、作業ディレクトリに転がっている未追跡ファイル（環境用 symlink 等）が
> そのままマージコミットへ入り、通常の差分レビューでは「マージだから」と見過ごされやすい。
> 実際、`git show <merge>^1:<path>` と `^2:<path>` の双方が「存在しない」にもかかわらず、
> マージコミット自身には存在するという状態になり、本チェックアウトを破壊した。
> 手順 4 の確認は、マージ時こそ省略してはならない。
