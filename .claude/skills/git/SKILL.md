...
> **手順 2 を省略すると起きること**: `git add -A` は作業ディレクトリの未追跡ファイルを無差別に拾う。
> 環境用に張った symlink（中身が絶対パス）が混入すると、別チェックアウトへ展開された時点で
> 「自分自身を指す symlink」になり、参照先の実体が失われる。過去に本番のサーバが起動不能になった。
...
> **マージ特有の危険**: マージコミットは**両親のどちらにも無い変更を含められる**。衝突解決中に
> `git add -A` を打つと未追跡ファイルがそのまま入り、「マージだから」と差分レビューで見過ごされる。
> 手順 4 の確認は、マージ時こそ省略しない。
