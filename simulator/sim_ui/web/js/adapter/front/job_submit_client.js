...
//
// 射程（Phase 9 S1 以降・§19.2）: 本 UI の投入経路（M5 sim_submission_builder）はこの 2 つを
// **一度も渡さない**——UI 出口を撤去したためである。したがって画面から出る本文では常に
// キー不在の枝しか通らない。それでも受け口を残すのは、ここが「sim のジョブ面 API に本文を
// 渡すクライアント」であって「この画面専用の投入器」ではないからである（API 面では機能が
// 存続する＝§19.2 の撤去はあくまで UI 出口のみ）。受け口を消すと、API 側に在る契約が
// クライアントから見えなくなる。
