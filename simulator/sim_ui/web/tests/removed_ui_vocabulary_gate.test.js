...
const FORM_PANEL_IDS = [
  ...
  "#simRunStatusPanel",
  // ISSUE-508 段階 3（§6.6.2）: 実行トレースの指定面。面である以上、整形は同じ規律に
  // 従う——選択子はこの id の配下に閉じる。
  "#simTracePanel",
  // ISSUE-441: 投入フォームの版面（設定 / 入力の 2 列）。**フォームだけが持つ id** であり、
  // 結果ビューア（`?job=<id>`）の DOM には現れない＝規律（波及 0）はそのまま成立する。
  // 版面を許さないと「面を並べる規則」を書く場所が body しか無くなり、結果ビューアの
  // レイアウトまで動く（この規律が禁じている当のもの）。
  "#simRunForm", "#simRunFormSettings", "#simRunFormInputs",
];
