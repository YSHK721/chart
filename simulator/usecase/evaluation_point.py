...
class EvaluationPoint:
    ...
    #: ティックモデルが供給した、この点のティックの時刻。**ティックを持たない
    #: スケジュールでは None**（バー粒度の点、およびティックが 1 本も無いバーの
    #: 持ち越し点）。合成疑似ティック（O→H→L→C 展開）は `bar.time` を運ぶ——それは
    #: 欠落ではなく真値であり（実 MT5 の OHLC 疑似ティックは分未満の時刻を持たない）、
    #: バー内の順序は `tick_ordinal` が表す。供給された値を加工せずそのまま運ぶ
    #: （判別子を置かない・設計書 §5.1）。数値評価には一切用いない（載せても指紋は
    #: 動かない）。時刻の正規化規則は `simulator/domain/bar_time.py` の epoch_seconds
    #: が単一ソースである（本モジュールは正規化を行わないため import しない）。
    tick_time: Any = None
