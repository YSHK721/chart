...
固定する不変条件:
    ...
    2. 両ツールのソースに銘柄仕様 8 キーへの数値リテラル束縛が 1 つも無い（AST 走査）。
       keyword 引数・単純代入（クラス属性を含む）・dict リテラルの 3 形を検出する。
...
from simulator.tools import export_trade_markers as markers_tool
...
def test_scanner_detects_a_reintroduced_literal():
    """**負の対照**: 撤去した 3 形を書き戻すと検出される（走査が機能することの実証）。"""
