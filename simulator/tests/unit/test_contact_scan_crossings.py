"""bit 一致回帰: crossings.detect_crossings の sign 規約（参照実装 prototype_260626-01）。

符号: val>level→+1 / <→-1 / ==→0(タッチ)。直近非ゼロ符号と異なる非ゼロ符号で 1 イベント。
direction=up(+1)/down(-1)。prev=series[j-1]。タッチは基準符号を更新しない。最初の非ゼロは
基準確立のみ（イベントなし）。連続フラット無発火。
"""
