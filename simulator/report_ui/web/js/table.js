...
// SPEC §2.2.2 の 11 列を trades[] を一次ソースに描画する（Symbol 列は meta.symbol 射影）。
...
// SPEC 11列に Profit 列は含まれないため format ヘルパ依存はない（trades[] から SPEC 列へ射影）。

// SPEC §2.2.2 11列: [key, label, kind]。kind は描画/比較の型ヒント（time/num/txt/side）。
...
export function buildTradeTable(hostTable, segment, linkage) {
