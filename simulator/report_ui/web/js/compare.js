...
import { cfmtLocale, signClass } from "./format.js";
...
function _renderCharts(data) {
  ...
  // エクイティ IS/OOS 重畳（残高曲線・split 前 init から起点）。
  ...
  if (eqEl) {
    const mk = (seg, col) => {
      ...
      return {
        ...
        borderWidth: 1.4, pointRadius: 0, tension: 0, fill: false, parsing: false,
      };
    };
    cmpCharts.eq = new Chart(eqEl.getContext("2d"), {
      ...
      options: {
        ...
        scales: {
          x: { type: "linear", ticks: { color: "#8b949e", maxTicksLimit: 9 }, grid: { color: "#1b2230" } },
