// global_fetch_binding.test.js — グローバル `fetch` を捕まえる箇所は必ず束縛する（ISSUE-350）。
//
...
test('検出器の自己検査: 既知の不良形を実際に検出する（検出しない検出器を置かない）', () => {
  // Arrange: 左が ISSUE-350 の不良形、右が是正後の形。
  const bad = [
...
test('グローバル fetch を捕まえる箇所は必ず globalThis へ束縛する（ISSUE-350 の再発防止）', () => {
  // Arrange
  const offenders = [];
  // Act
