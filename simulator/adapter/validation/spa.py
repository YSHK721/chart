...
class HansenSpa:
    def spa_pvalue(
        ...
        *,
        ...
        omega = _bootstrap_std(F, seed, B, block)
...
def _bootstrap_std(F: "np.ndarray", seed: int, B: int, block: int) -> "np.ndarray":
...
def _flat_top_weight(s: float) -> float:
...
def _autocorr(xc: "np.ndarray", c0: float, k: int) -> float:
...
def _pw_block_len_one(x: "np.ndarray") -> float:
    ...
    m_hat = K_n
    for m in range(1, n):
        run = [abs(_autocorr(xc, c0, m + j)) < crit for j in range(1, KN + 1) if m + j < n]
        if run and all(run):
            m_hat = m - 1
            break
    ...
    for k in range(1, M + 1):
        w = _flat_top_weight(k / M)
        rk = _autocorr(xc, c0, k)
...
def _pw_block_len(F: "np.ndarray") -> int:
    """Politis-White(2004) 自動ブロック長（各候補列の b_opt の中央値・D3）。"""
    n = F.shape[0]
    bs = [_pw_block_len_one(F[:, j]) for j in range(F.shape[1])]
