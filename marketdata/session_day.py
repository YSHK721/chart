...
（dwell/zp/tf-period/rollup/frontend 供給値）は本モジュールを唯一の規則源として参照する
（再実装を禁ずる）。

この宣言は ``marketdata/tests/test_module_dependency_declarations.py`` が AST 走査で強制する。
"""
...
def period_session_labels(tf: str, label: str) -> "list[str]":
    ...
    期間先頭ブローカー日の暦算術は台帳（:func:`marketdata.tf_ledger.period_first_ymd`）へ委譲する
    （ISSUE-479 M-3）。対応 tf の集合も台帳の導出値であり、ここに tf 名を書き写さない。
    """
    y, m, d = (int(x) for x in str(label).split("-"))
    end = datetime(y, m, d).date()
    if tf not in _tf_ledger.CALENDAR_LABEL_CODES:
        raise ValueError(f"period_session_labels: 1W|1M のみ対応: {tf!r}")
    first = _tf_ledger.period_first_ymd(tf, y, m, d)
    out = []
    cur = first
    while cur <= end:
