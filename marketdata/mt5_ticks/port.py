...
class SeriesWriteIncomplete(Mt5SupplyError):
    """系列の組へ**揃って書けなかった**（ISSUE-511 段階 8-D-2b の段 5・設計 D-6）。

    2 つ以上のファイルを原子的に書く手段は無いため、組の一部だけが書けた状態・組の先端が
    揃わない状態は起こりうる。起こったときに素の例外で落ちると、常駐はトレースバックを吐いて
    未知のクラッシュに見え、**どの系列がどこまで進んでいるか**が運用者に残らない。よって
    「どの ref に何本書き、どの ref が未書込か」を文面に載せてここで止める。

    :class:`Mt5SupplyError` の派生であるのは、待っても直らない側（Fail-Stop）であり、かつ
    常駐の捕捉集合（``tools/mt5_tick_watch.py`` の周期の ``except``）に**既に載っている**型だから
    である。新しい根の型を足すと、捕捉集合へ書き足すまでの間だけ格下げされない経路が生まれる。
    """
