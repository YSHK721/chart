...
描画データの型（numpy / pandas）と自パッケージのみで、``simulator`` / ``indigators`` /
``dashboard_ui`` / ``unified_ui`` / ``marketdata`` / ``tools`` といったアクター、および計算層
``common`` を参照してはならない（参照すると共有プリミティブが特定アクターに縛られ、
安定度逆転・循環の起点になる）。

現状すべて緑＝**回帰錨**である（違反を直すためではなく、違反の混入を検出するために置く）。
検査に力があること（合成ソースで違反を実際に検出し、非違反を誤検出しないこと）を併せて固定する。

走査ヘルパはリポジトリのアーキ回帰テストの既存様式（``test_contact_scan_usecase_purity.py`` /
``test_replay_purity.py``）に倣い各テストモジュールで自己完結させる。パッケージ独立性を検査する
