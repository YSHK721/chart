...
既存の日別 parquet は**上書きしない**（在る日は読み飛ばして報告に出す）。
...
def select_months(
    ...
    """
    start, _, end = (months or "").partition(RANGE_SEPARATOR)
    selected = []
...
def run(args: argparse.Namespace) -> int:
    ...
    LOG.info(
        ...
        report.days_empty, report.rows_unaccounted,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--months", default=None,
                        help="取り込む年月の範囲 FROM:TO（両端含む・片側省略可。既定は全部）")
