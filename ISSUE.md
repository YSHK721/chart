...
`.doc/PRICE_LEVEL_REACH_SHEET_BASIC_DESIGN.md` v0.3.0 が唯一の仕様源。要点:

- 行＝**役割 23 行**（chart 追従 11 ＋ MTF 固定 12）・列＝時間足 8・並びは価格降順の単一並び。
  MTF 固定水準は空欄を並べず、**全列にまたがる帯 1 本**で描く（値が 1 つ＝全時間足に等しく効く）。
