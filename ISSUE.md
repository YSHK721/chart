...
- **ステータス**: OPEN（2026-08-29 起票・**設計案提示／1 点承認待ち・実装未着手**）
...
1. **要承認**: 価格セルの背景を**地平 3 段で 3 分割**するか（推奨・§4.3 と同じ区分なので
   新しい概念が増えない）、**1 色**にするか（最も極端な段のみ・情報は減るが最も軽い）。
2. **未測定**: 段の**時間的な安定性**。§4.3 の「指名の持続」に相当する量を段については
   測っていない。表示に添えるなら着手前に実測する。**断定しない。**
3. 走行 H/L の更新頻度（＝係数の再計算頻度）を実測していない。1m での更新率で費用が決まる。
