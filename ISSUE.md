...
## ISSUE-380: [不具合] ライブ /compute の滞留で指標が全滅し legend が左上縦積みに縮退する（2026-08-12・IN_PROGRESS）

- **検出日**: 2026-08-12
- **検出経路**: 依頼者報告「指標の表示位置が過去の状態に戻っている」（ライブ 4h・16 指標のスクリーンショット）。
  背景ジョブによる実 UI（Playwright・統合 8000）＋サーバ実測で原因確定。
- **症状**: ライブ画面で全指標の legend が左上に 1 ボックス縦積みになり、ペイン指標
  （ma_marod・RSI・ティックボリューム等）の専用ペインが生成されない。見た目は
  ISSUE-276 以前の旧 legend（#legend 縦積み）と同一のため「過去の状態に戻った」ように見える。
- **実測（2026-08-12 13:28–13:37 UTC）**:
  1. 配信元は本チェックアウト（router 8000 → live 8001、いずれも /workspaces/app・develop 最新）。ツリー違いではない。
  2. `live:indicatorUi.applied.v1`（localStorage）は健在＝**保存された表示位置・並び順は失われていない**。
  3. 8001 `GET /candles`（4h）は 0.27 秒 200・最終バー 2026-08-12 08:00 UTC＝正常。
  4. 8001 `POST /compute` は軽量指標（moving_averages・limit=50）でも 60 秒無応答。
  5. サーバプロセス（11:47:53 起動）はスレッド 120 本。compute-pool 3 スレッド（12:28:38 生成）が
     GIL を取り合いながら連続実行中（futex アドレス同一・カウンタ前進＝デッドロックではなく重計算の直列消化）。
  6. ポート 8001 に CLOSE_WAIT 88 本＝**クライアントが 30 秒タイムアウトで去った後も、その計算が
     キューに残り 3 ワーカーを占有し続けている**（ISSUE-257 で裁定済みの「捨てられる計算」型）。
     接続数は 60 秒間で 138→142 と増勢＝流入が処理を上回り続ける自己増悪。
  7. 再現: applied に 6 指標（ペイン 3 種含む）を種付けしてリロード → 全 compute 30 秒タイムアウト →
     `.pane-legend` が 1 個（top:77px）に全 6 行縦積み＝依頼者スクリーンショットと同一の縮退表示。
- **機構（フロント側の縮退経路・コード確認済み）**: compute 未完了だと series が描かれず
  renderer に slot.pane が生成されない → `chart_renderer.js:818-824 _slotPaneIndex` が全行 paneIndex 0 を
  返す → `pane_legend_view.js:172-178` が全行を pane 0 の 1 グループへ集約＝縦積み。
  表示位置の決定・永続化・復元ロジック自体は直近 6 コミットで無変更（Phase 4 は sim 専用ファイルのみ）。
- **根本原因**: サーバがクライアント切断を検知せず、破棄済みリクエストの計算を無期限に
  キュー消化し続けること（仕事の量が無制限）。フロントの 30 秒タイムアウト再試行と組み合わさり、
  流入 > 処理の恒常化で新規 compute が事実上永久に応答しない。
- **対策案（抜本・ISSUE-257 裁定「捨てられる計算を消す」の実装）**:
  1. 段階 1: /compute のキュー取り出し時に接続生存を確認し、切断済みリクエストは計算せず投棄する。
     通過条件: CLOSE_WAIT 滞留下で新規 compute の応答時間が「実計算時間＋現役キュー分」に収束すること。
  2. 段階 2: 同一 (indicatorId, variant, params, datasetRef, timeframe, limit) の実行中計算へ後続を合流させ、
     同一計算の重複実行を消す。通過条件: 同一 16 指標の同時 2 クライアントで compute 実行回数が 16 回のみ。
  3. 表示位置は保存済みのため、サーバ側是正後のリロードで自然回復する(フロント変更不要)。
- **実施（2026-08-12・段階 1+2 実装済み）**: `framework/server.py` に
  `_make_client_gone_probe`（select+MSG_PEEK の非ブロッキング切断検知）・
  `_compute_unless_abandoned`（実行直前の生存確認・全依頼元切断なら計算せず投棄）・
  `_run_coalesced`（同一 key の実行中計算へ合流・完了で登録解除＝キャッシュではない）を追加し、
  `/compute`（専用ワーカー・プール両経路）と MP 3 経路（`_respond_mp_via_worker`）を結線。
  ISSUE-259 の分担（計算はワーカー・応答書き出しはリクエストスレッド・module 関数規律・
  `_MP_WORKER` 参照 1 箇所）は不変（構造ガードテスト全通過で担保）。
  テスト: 新規 11 件（probe 生存/切断/破棄・投棄・合流・登録解除・例外伝播・
  スナップショット後合流の再実行・HTTP 実経路の投棄/合流）＋ API スイート 555 件全通過。
  残: サーバ再起動（serve.sh・ユーザー操作)後の実 UI 実測で RESOLVED 化。

## ISSUE-381: [不具合] 1m チャートで moving_averages が全滅する（1M MTF の O(N×M) 再構築・2026-08-12・OPEN）

- **検出日**: 2026-08-12
- **検出経路**: 依頼者報告「1分足移動平均線が表示されない」＋依頼者提供のリプレイ診断ダンプ
  （12 指標中 moving_averages 6 本と market_profile のみ「スロットなし（未描画）」）。
  背景ジョブが同一 12 指標構成を実 UI（Playwright・統合 8000）へ種付けして完全再現。
- **症状**: ライブ 1m・12 指標構成の復元で、moving_averages 6 本（1M MTF ×3・chart EMA60 ×3）
  だけが全て「計算失敗（当該インスタンスのみスキップ）: 応答タイムアウト（30000ms）」になり
  未描画。他 5 指標（ma_marod・btlm_trail・btlm_trail_marod・profit_rsi・tickvol）と MP は描画成功。
  **再起動直後の空いたサーバでも決定論的に再現**（2026-08-12 22:11 実測・console 6 件のタイムアウト）。
- **実測（2026-08-12）**:
  1. 単発では全指標が軽い: ma_marod 0.43s / btlm_trail 0.44s / btlm_trail_marod 0.45s /
     profit_rsi 0.41s / tickvol 0.40s / MP(zp) 0.27s / chart EMA60 0.48s（いずれも 1m×1500・8001 直測）。
  2. **1M（月足）MTF MA だけ毎回 8.8〜9.2 秒**（初回・再送とも同等＝リクエスト間でキャッシュ無効）。
  3. cProfile（プロセス内 2 回目・計 19.4s ※プロファイラ倍率込み）: `causal_mtf_frames`（
     `adapter/compute/mtf_causal_frames.py:65`）が 19.1s を占有。内訳は `frame_from_bars` →
     `latest_compute` → incremental moving_averages `prepare` が **8,915 回**呼ばれる
     O(投影バー数 × 状態再構築) の全再計算。
  4. 帰結: 1M MTF ×3 本（source hlc3/high/low）が計算プール 3 枠を各 ~10 秒以上（GIL で実質直列
     ≈30 秒）占有 → 後続の chart EMA60 ×3 が待ち超過 → **6 本全てフロントの 30 秒制限で
     スキップ（再試行なし）**＝「1 分足で MA が表示されない」。
  5. パラメータ正当性は確認済み: 同一ボディの単発 curl は全て 200・正値（1M MTF は
     `computeTimeframe:"1M"` で月足 EMA12=57,624・stepped=true と正しい）。
- **根本原因**: `causal_mtf_frames` の因果的 MTF 投影が、要求のたびに全バーについて上位足 MA
  状態をゼロから再構築する O(N×M) 実装で、かつ結果・中間状態をリクエスト間で記憶しない。
  仕事の量が構成上過大（ISSUE-257 の裁定「減らすのは仕事の量」に反する実装）。
- **対策案（抜本・段階分割）**:
  1. 段階 1: `causal_mtf_frames` の増分化 — 上位足バー列に対する incremental `prepare` の
     状態を前進走査で引き継ぎ、O(N) で全フレームを構成する（因果性は既存
     `test_mtf_causal.py` / 検定 CLI で担保。通過条件: 出力が現行実装とバイト一致し、
     1M MTF 単発が 1 秒未満）。
  2. 段階 2: (ref, computeTimeframe, params) キーの投影メモ — 確定部分をリクエスト間で保持し
     forming 分のみ再計算（通過条件: 再送が ~ms・12 指標バーストの全所要が 30 秒未満で
     全 12 描画）。
  3. ISSUE-380 の修正（切断投棄＋合流・未マージ）と併せて適用（バースト時の重複実行も消える）。
- **備考**: フロントの「タイムアウト後に再試行しない」挙動は本症状の増幅要因だが、原因は
  サーバ側の仕事量であり、サーバ是正で症状ごと消える（フロント変更なし）。
