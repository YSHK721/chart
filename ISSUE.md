...
- **検定**（`indigators/indicator_ui/api/tests/test_mp_worker_io_separation.py`・10 件）: 実行時 2 種（3 経路 × parametrize）＝ 計算スレッド ident が 2 要求間で同一（直列化維持）かつ書き出しスレッド ident と交わらない／ワーカー内例外が同一の 500 nested error になり書き出しはリクエストスレッド。構造 4 種（AST）＝ ワーカー送致（`_MP_WORKER`/`_COMPUTE_WORKER`/`_COMPUTE_POOL` の `run`/`submit`）の引数式に `self` が現れない／`_MP_WORKER` の参照点が共通殻 1 箇所のみ／送致される計算が module 関数でありソケット API（`self`/`_send_json`/`wfile`/`send_response`/`send_header`/`end_headers`）に触れない／その `return` が必ず 2 要素タプル。
- **識別力の実証**: 是正を**全巻き戻し**（`git checkout` で旧 server.py）→ **10/10 Red**。**部分退行**（1 経路だけ共通殻を迂回し `_MP_WORKER.run(lambda: self._send_json(*_compute_tf_period_profile(...)))`）→ **6 Red / 4 pass**、しかも Red になる実行時検定は当該 `tf_period_profile` の 2 件のみ＝経路粒度で識別する。
...
- **回帰**: indicator_ui api + market_profile api **1218 passed / 0 failed**（是正前ベースライン 1208 passed に新規 10 件を加えた数と一致）。indicator_ui web 1100 pass / market_profile web 319 pass（いずれも fail 0）。
