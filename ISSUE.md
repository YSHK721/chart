...
2. 帯上端より外側の目盛りは**経験順位ではなく既存の POT/GPD 水準で与える**ことを §5.3 に明記する
   （欠陥 1）。
3. §5 へ「**積み上がる量は同じ経過割合の分布へ当てる**」を規約として追加し、`tickvol` を
   その唯一の該当指標として挙げる（欠陥 2）。
4. §9 の未確定へ「`1M` は窓 260 本に対しバー 169 本しかなく `p` が劣化する。窓を縮めるか
   `1M` を連続配色の対象から外すか」を追加する。
...
- **裾の目盛りの当てはめ方が未検証**。GPD で帯外を解像する方針は決めたが、
  `p` へどう写すか（`q_high` を 0.90 に固定して外側を GPD の CDF で埋めるか等）を実測していない。
  **断定しない。着手前に測る。**
- **`4h` 以上の形成中バーのバイアスが未測定**。M4 は 1m から部分足を再構成できる
  `5m` / `15m` / `1h` に限る（`4h` 以上は 52 日では窓 500 本に足りない）。
  傾向は `5m` → `1h` で単調に悪化しており `4h` 以上はより強いと**推測されるが、未測定**である。

### 関連

ISSUE-449（第 2 表の構造 §5.1〜5.4）・ISSUE-453（オシレータの価格投影・`tickvol` は対象外）・
ISSUE-233 / ISSUE-262（増分器の宣言と無言縮退＝GPD 未出力の同型）。
測定スクリプト: `tools/measure/issue449/probe_continuous.py`（M1〜M3）/
`probe_forming.py`（M4）（commit `e2fed68`）。
