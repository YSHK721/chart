...
`.doc/PRICE_LEVEL_REACH_SHEET_BASIC_DESIGN.md` v0.1.0 が唯一の仕様源。要点:

- 行＝一意な水準（指標名では固定しない）・列＝時間足 8 ＋ MTF 共通 1・並びは価格降順。
...
- 対象テンプレートは 7 種・8 時間足へ紐付け。価格スケール指標は延べ 73 本。
- 水準は chart 追従 26 種（延べ 40 セル）と MTF 固定 12 種（延べ 33 セル）に分かれる。
  MTF 固定は表示時間足に依らず値が同一のため 1 列へ畳める。総セル **73 → 52**。
- `moving_averages` は全時間足通算 27 種あるが各時間足は 6〜9 種のみ。指標名を行に固定すると約 7 割が空欄。
