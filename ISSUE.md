...
- **ステータス**: OPEN（2026-07-29 起票・**現状は仕様 §4.1-6 の FAIL 行に沿って動作／恒久方針は未裁定**）
...
- **表示形式の追補（2026-07-30・ユーザー指摘「ジグザグで視認性が悪い」）**: バー毎の帯を線で繋ぐことに情報上の意味がほぼ無いことを実測で確認し、**既定を水平ライン（最新水準のみ）へ変更**した。
  ...
  - `display_mode` パラメータを新設。`levels`（既定・最新水準を価格軸ラベル付き水平線で 1 組）／`bands`（従来のバー毎の帯・検証用）。
  - 実 UI 実測（5 分足）: 極端+ 63,079.61 / 外れ値+ 62,302.42 / +2σ 62,275.12 / +1σ 62,197.51 / 価格 62,120.08 / −1σ 62,042.62 / −2σ 61,965.32 / 外れ値− 61,934.54 / 極端− 61,711.40（単調・コンソールエラー 0 件）。
...
- **ステータス**: OPEN（2026-07-30 起票・バンド表示の検討中に検出・**本エージェントが原因まで特定**）
