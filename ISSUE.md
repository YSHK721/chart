...
- **対応記録（W2・🟡-2・2026-07-16）**: `VolBandRepositoryPort`（save/save_all/get/all_week_ids の4メソッド）を `VolBandWriterPort`（save_all）／`VolBandReaderPort`（get）へ read/write ロール分離。書込クライアント estimate_weekly_band は Writer・読取クライアント run_weekly_segments は Reader を受ける形へ。production 呼び出し0件の save 単体・all_week_ids は削除（旧 all_week_ids 契約固定テストは同カテゴリで削除）。具象は両ロール統合実装のまま充足。simulator 962×2 緑。未着手: 🟡-3/-4（TimeframeController/MarketProfileController の host 全体依存＝symlink 共有ベース制約で即時分割困難・後続）・🟡-5（MarketProfileHistogramPrimitive の god interface＝web/js・後続候補）・🔵。
