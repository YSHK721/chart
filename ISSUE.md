...
- **ステータス**: OPEN（2026-07-18 起票。HEAD（全変更 stash 退避）でも同一 6 件が fail することを実測確認済み＝ISSUE-120 変更とは無関係の既存失敗）
...
- **対策案**: ①はセッション日基準の期待値へ更新（sessionDayStart 由来の値で再計算）。②は MP 既定 params の実供給値を実測して期待を是正 or 実装欠陥なら修正。着手指示待ち。
...
- **申し送り（未対応・小）**: 既存 `jp225_tick_m1.csv` の重複行 1 件は serving 無害化(2)＋front 防壁(3)で無害だが、CSV 実体には残存（/compute の当該 1分バーが軽微に不正確）。恒久除去は次回 rollup 全再構築（build 経路が(1)で dedupe）で自然解消、または承認の上で当該行の除去＋rollup 再生成（ISSUE-107 と同手順）。
