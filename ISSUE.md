...
**未検証（取り込み設計の前に潰す）**:
- **T8**: 月境界の重複。ページ注記「2 つ以上連続した月をインポートすると末日が重なる」＝重複排除規約が要る（本実測は 1 か月のみで未確認）。
- **T9**: ページ注記「サーバーメンテナンス等でデータがない期間は**ニューヨークサーバー配信**のティックを使用」＝一部区間に別 venue の気配が混入する。識別方法が未検証。
- **T10**: 2012-06〜2020-04（Dukascopy 木が持つ 8 年）は本経路でも埋まらない。よって「置換」は 2020-05 以降に限った話であり、全期間の置換は依然不成立。
