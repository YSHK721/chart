...
- **ステータス**: IN_PROGRESS（2026-07-14 単位①スキャン完了・bp 幅の依頼者裁定待ち）
...
- **裁定待ち（bp 幅の選択）**: (a) **1.5bp（推奨）**＝現在価格で現行 10pt と同等（67,000×1.5bp≈10pt）・挙動を時間不変化するだけで今日の統計密度を維持 (b) 1bp / 0.5bp＝校正上安全だが単日タイルの z 振幅が縮む（15m の精細分析要望には合致）。選択後、単位②（production の log 格子化・キャッシュ版数・tf-period zp・フロント unit）へ。
