...
- **ステータス**: IN_PROGRESS（2026-08-28 起票・第 1/2 段の是正を実施済み・値一致を実測で確認。
  サーバ再起動で反映。第 1〜3 段の是正済み。第 4 段＝原因 G と起動経路の床が未着手）
...
- 原因 G（DataFrame ⇄ dict の往復）。`latest_seq` の契約変更を伴う。
- 起動経路の床（MTF 1 本あたり約 0.26〜0.36 秒）。内訳は `load_dataframe` と
  `bars_from_frame(df_source)` が主と推定（**未計測**）。
