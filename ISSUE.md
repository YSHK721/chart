...
**持続は指名の安定性であって、当たるかどうかとは無関係**。到達率・反応は**未測定**であり、
「目標として使える」という判断は基準を定義せずに述べたため撤回した（設計書 §9-1）。
...
1. **到達率・反応が未測定**。ISSUE-449 で測ったのは「最も近い水準」の**指名の持続**だけであり、
   そこへ価格が届くか・届いた後に止まるかは測っていない。指値は届かなければ約定しない。
   最低限: 指名された水準への到達率／指名から到達までの時間／到達後に反転する割合。
