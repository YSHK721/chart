...
- **重大度**: 中（第 2 表の配色規約が実装と食い違う。実装前に直せば描画の作り直しは生じない）
...
### 是正案（要承認・設計書のみ。実装・`levels.py` は無改変）

1. §5.3 の該当 1 行を、上記「正しい段の構成」の図と、上側 4 段・下側 1 段の非対称の明記へ差し替える。
2. §5.4 の版面イメージの `tickvol` 行を実測値に合わせ、`静穏` を含む形へ直す。
3. §5.2 へ「水準が NaN のセルは段を持たない。空欄ではなく**水準なし**と明示する」を追加する。
4. §9 へ「`tickvol` の段の名前（`静穏` / `med 超`）が他 3 種と非対称であることを UI 上どう示すか」
   を未確定として追加する。
