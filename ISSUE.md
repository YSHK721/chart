...
## ISSUE-321: [計測誤り] §2.2 E-21 のアプリ UI 色リテラル計測が陳腐化し、8 桁 hex を構造的に取りこぼしていた（2026-08-09）
...
## ISSUE-322: [制約] Market Profile の TPO バーは HSL 色相ランプのためテーマ 2 色から再現できず、v0.4.0 で変更対象外に残す（2026-08-09）
...
## ISSUE-323: [欠陥] 導出既定値 `muted` が混合比で定義されており、地を変えると自分の診断 W-C2 を割り込む（2026-08-09）
...
## ISSUE-324: [設計入力の誤り] 導出表の `range` 規則「primary と alert の中間」が逆算不能だった（2026-08-09・RESOLVED）
...
## ISSUE-325: [欠陥] serve.sh の二重起動判定が「応答の有無」だけを見るため、別ツリーの残存スタックが黙って勝つ（2026-08-09）
...
- **同一機構の再発**: ISSUE-317（「初期化に失敗しました: controller.setColorThemeProvider is not a
...
## ISSUE-326: [欠陥] 導出 `text` が混合比のままで対地コントラストの下限を持たず、最も読ませる語が図の下限すれすれになる（2026-08-09）
...
- **根本原因**: `text = mix(surface, 対比側, 0.820)` が**混合比**で定義されており、対地コントラストに
  ...
  ISSUE-323 が `muted` について指摘したのと**同一の病因**であり、5-C-1 では `level` / `muted` の 2 語
