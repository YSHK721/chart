...
## ISSUE-197: [仕様の矛盾] CVFE `quality_gate = "FAIL"` 経路は §3.3 E07 と §3.3 E08 が両立しない（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**実装側は暫定解決済み／仕様の裁定は未取得**）
...
- **関連**: ISSUE-199（同じ x4 の退化が標本側の理由でも起きる）。

## ISSUE-198: [仕様の欠陥・実測] CVFE の TSRV が既定 `K = ceil(n^(2/3))` で 8.9% 過小になる（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**TBD-1 の裁定待ち。実装は仕様どおりで変更しない**）
...
## ISSUE-199: [仕様の欠落] CVFE の HAR は学習窓にジャンプが 1 本も無いと必ず E08 で停止する（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**実装側は暫定解決済み／仕様の裁定は未取得**）
...
- **暫定対応（実装側）**: ISSUE-197 と同一機構。x4 が学習標本内で厳密に定数なら当該列を外して `β4 = 0` に固定し、`W04_HAR_JUMP_COLUMN_CONSTANT` を WARN 出力する。
- **要裁定**: §4.5 に「x4 が定数のとき β4 = 0 とする」を明記するか、`n_har` の下限をジャンプ検出本数の期待値から導出し直すか。

## ISSUE-200: [仕様の曖昧性] CVFE §9 段階 2 の `ω/σ` の尺度が未定義（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**解釈を明示して実装／仕様の裁定は未取得**）
...
## ISSUE-201: [仕様の内部不整合] CVFE §4.1-6 の DEGRADED 閾値 0.50 は §9 段階 2 の要求を満たさない（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**閾値は変更していない**）
- **事象**: 仕様 §9 段階 2 は「`ω/σ ≥ 0.5` のとき `"DEGRADED"` 以上を返す」ことを要求するが、§4.1-6 の閾値は `S > 0.50` であり、`ω/σ = 0.5` ちょうどでは届かない。
- **実測（2026-07-29・ISSUE-200 の解釈 (a)・520 バー）**:
...
## ISSUE-202: [仕様の過少規定] CVFE §9 段階 2 の DGP では M4 が M3 に勝てない（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**実装は妥当と実証済み／仕様の裁定は未取得**）
...
## ISSUE-203: [仕様の未定義域] CVFE §4.7-1 のギャップ判定は `delta_star_sec = 0` で全バーが該当する（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**仕様の式をそのまま適用／裁定は未取得**）
...
## ISSUE-204: [既存の構造問題] indigators 配下の指標パッケージは同一 pytest セッションで同時実行できない（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**cvfe 実装中に検出。cvfe が持ち込んだものではない**）
...
## ISSUE-205: [仕様の欠落] CVFE 学習窓の内側に空バー（E06）があると必ず E08 で停止する（2026-07-29）
...
## ISSUE-206: [仕様の欠陥] CVFE §3.1 の下限 `N = n_har + 22` では出力が 0 本になる（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**下限は変更していない**）
...
## ISSUE-207: [仕様の未定義域] CVFE ギャップ EWMA 初期値が 1 本の `nan` で恒久的に死ぬ（2026-07-29）
- **ステータス**: RESOLVED（2026-07-29 起票・**実装側で解決済み／仕様への追記は未裁定**）
...
## ISSUE-208: [仕様と因果性の矛盾] CVFE ギャップ EWMA 初期値がギャップ保有バー 200 本未満のとき将来を参照する（2026-07-29）
...
## ISSUE-209: [仕様の欠陥・実測] CVFE の TSRV 経路でジャンプ誤検出率が許容の 45〜56 倍になる（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**式は変更していない**）
...
## ISSUE-210: [仕様と因果性の矛盾] CVFE §4.7-1 のギャップ判定は当該バーのティック時刻を要する（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**仕様の式をそのまま適用**）
...
- **関連**: ISSUE-203（同じ §4.7-1 の `delta_star_sec = 0` 問題）。

## ISSUE-211: [仕様の曖昧性] CVFE §9 段階 1 の「5σ ジャンプ」の σ の尺度が未定義（2026-07-29）
- **ステータス**: OPEN（2026-07-29 起票・**解釈を明示して実装**）
...
- **関連**: ISSUE-200（§9 段階 2 の `ω/σ` にも同種の尺度未定義がある）。
