...
④含む構造:
    ...
    スケールを共変量の関数とする GPD（log β = Xγ・ξ 一定）とその尤度比検定・検出力曲線は
    本モジュールにはない。研究ゲート専用のため common の gpd_covariate へ分離した
    （ISSUE-479 Wave2 C-2。本番参照 0 の探索コードを常時 import 経路から外す）。
    最適化ルーチン（Nelder–Mead）も common の nelder_mead へ抽出した。

⑤依存: 標準 __future__ / dataclasses / math / 外部 numpy /
    プロジェクト内 common の nelder_mead（最適化ルーチン）。
"""
...
XI_EPS: float = 1e-8
#: 旧 private 名（同一オブジェクト。既存の参照を無改変に保つ）。
_XI_EPS = XI_EPS

#: 最適化ルーチンの旧 private 名（実装は common の nelder_mead へ抽出済み・同一オブジェクト）。
_nelder_mead = nelder_mead
